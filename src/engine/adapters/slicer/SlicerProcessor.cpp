...
namespace dstools {
    ...
    static TaskProcessorRegistry::Registrar<dsfw::SlicerProcessor> s_reg(QStringLiteral("audio_slice"),
    ...
    dsfw::Result<void> SlicerProcessor::initialize(dsfw::ModelManager & /*mm*/, const dsfw::ProcessorConfig & /*config*/) {
        return dsfw::Ok();
    }
    ...
    dsfw::Result<dsfw::TaskOutput> SlicerProcessor::process(const dsfw::TaskInput &input) {
        dsfw::SlicerService slicer;
        ...
        for (const auto &[startSample, endSample] : sr.chunks) {
            const dsfw::TimePos inPos = secToUs(static_cast<double>(startSample) / sr.sampleRate);
            const dsfw::TimePos outPos = secToUs(static_cast<double>(endSample) / sr.sampleRate);
