...
dsfw::Result<std::vector<std::pair<float, float>>>
WordList::remove_overlapping_intervals(const std::pair<float, float>& raw_interval,
    ...
    if (!(r_start < r_end)) {
        return dsfw::Err<std::vector<std::pair<float, float>>>(
...
void WordList::add_AP(const Word& new_word, float min_dur) {
    try {
        ...
        std::vector<std::pair<float, float>> ap_intervals = {{new_word.start, new_word.end}};
        ...
        ap_intervals.erase(std::remove_if(ap_intervals.begin(),
                                          ap_intervals.end(),
        ...
        std::cerr << "[HFA] ERROR in add_AP: " << e.what() << std::endl;
...
void WordList::fill_small_gaps(const float wav_length, const float gap_length) {
    try {
        ...
        std::cerr << "[HFA] ERROR in fill_small_gaps: " << e.what() << std::endl;
...
void WordList::add_SP(const float wav_length, const std::string& add_phone) {
    try {
        ...
        std::cerr << "[HFA] ERROR in add_SP: " << e.what() << std::endl;
    ...
    for (size_t i = 0; i < words_.size(); i++) {
        ...
        for (size_t j = 0; j < word.phones.size(); j++) {
            ...
            if (j < word.phones.size() - 1 && std::abs(word.phones[j].end - word.phones[j + 1].start) > 1e-6) {
                const std::string warning_msg = "Word '" + word.text + "' phone '" + word.phones[j].text + "' end(" +
                                                std::to_string(word.phones[j].end) + ") != next phone '" +
                                                word.phones[j + 1].text + "' start(" +
