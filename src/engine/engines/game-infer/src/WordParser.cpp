...
namespace Game
{
    ...
    std::pair<bool, std::string> validatePhones(const std::vector<std::string> &phSeq, const std::vector<float> &phDur,
                                                const std::vector<int> &phNum) {
        if (phSeq.size() != phDur.size()) {
            return {false,
                    "Length mismatch: " + std::to_string(phSeq.size()) + " phonemes vs " +
        ...
        if (totalPhNum != static_cast<int>(phSeq.size())) {
            return {false,
                    "Word span mismatch: sum of ph_num is " + std::to_string(totalPhNum) + ", expected " +
