...
namespace FunAsr {
    float *loadparams(const char *filename) {
        ...
        fclose(fp);
        if (n != nFileLen) {
            std::cerr << "Failed to read complete file: " << filename
                      << " (expected " << nFileLen << ", got " << n << ")" << std::endl;
    ...
    void softmax(float *din, int mask, int len) {
        if (mask <= 0 || len < mask) return;
        if (mask > 1000000) { // 合理的上限检查
            std::cerr << "softmax: mask too large: " << mask << std::endl;
            return;
        }
        float *tmp = (float *) malloc(mask * sizeof(float));
        if (!tmp) {
            std::cerr << "softmax: malloc failed for mask=" << mask << std::endl;
            return;
        }
        int i;
        float sum = 0;
    ...
    void log_softmax(float *din, int len) {
        if (len <= 0) return;
        if (len > 1000000) { // 合理的上限检查
            std::cerr << "log_softmax: len too large: " << len << std::endl;
            return;
        }
        float *tmp = (float *) malloc(len * sizeof(float));
        if (!tmp) {
            std::cerr << "log_softmax: malloc failed for len=" << len << std::endl;
            return;
        }
        int i;
        float sum = 0;
