...
Ort::SessionOptions OnnxEnv::createSessionOptions(
    ExecutionProvider provider, int deviceId, int interOpThreads) {
        ...
        case ExecutionProvider::DML: {
            ...
            if (getApiStatus.IsOK() && ortDmlApi) {
                ...
                Ort::Status appendStatus(
                    ortDmlApi->SessionOptionsAppendExecutionProvider_DML(options, deviceId));
        ...
        case ExecutionProvider::CUDA: {
            ...
            if (createStatus.IsOK()) {
                std::unique_ptr<OrtCUDAProviderOptionsV2,
                    decltype(ortApi.ReleaseCUDAProviderOptions)> cudaOptions(
...
std::unique_ptr<Ort::Session> OnnxEnv::createSession(
