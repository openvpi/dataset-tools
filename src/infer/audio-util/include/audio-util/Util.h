...
namespace AudioUtil
{
    ...
    /// @brief Read audio file and convert to mono float samples.
    /// @param filepath Path to the audio file (WAV format).
    /// @param samples Output vector to store mono audio samples.
    /// @param sampleRate Output sample rate of the audio.
    /// @param msg Output error message on failure.
    /// @return True on success.
    bool AUDIO_UTIL_EXPORT readAudioToMonoFloat(const std::filesystem::path &filepath,
