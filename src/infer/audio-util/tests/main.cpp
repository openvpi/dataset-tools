...
int main(int argc, char *argv[]) {
    if (argc < 3) {
        ...
        std::cout << std::endl;
        std::cout << "  Without --all: runs only TC-AU-001 (basic resample, compatible with original usage)"
                  << std::endl;
    ...
    const auto tmpDir =
        wav_out_path.parent_path().empty() ? std::filesystem::temp_directory_path() : wav_out_path.parent_path();
