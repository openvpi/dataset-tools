...
namespace AudioUtil
{
    ...
    Slicer Slicer::fromMilliseconds(int sampleRate, const SlicerParams &params) {
        ...
        int winSize = static_cast<int>(std::min(
            divIntRound<int64_t>(static_cast<int64_t>(params.minInterval) * sampleRate, 1000LL),
            static_cast<int64_t>(4) * hopSize));
        int minLength = static_cast<int>(divIntRound<int64_t>(
            static_cast<int64_t>(params.minLength) * sampleRate, 1000LL * hopSize));
        int minInterval = static_cast<int>(divIntRound<int64_t>(
            static_cast<int64_t>(params.minInterval) * sampleRate, 1000LL * hopSize));
        int maxSilKept = static_cast<int>(divIntRound<int64_t>(
            static_cast<int64_t>(params.maxSilKept) * sampleRate, 1000LL * hopSize));
    ...
    MarkerList Slicer::slice(const std::vector<float> &samples) const {
        ...
        if (silence_start >= 0 && static_cast<int64_t>(rms_list.size()) - silence_start >= m_minInterval) {
            const int64_t silence_end =
                (std::min)(static_cast<int64_t>(rms_list.size() - 1), silence_start + static_cast<int64_t>(m_maxSilKept));
        ...
        if (sil_tags.empty()) {
            ...
            if (sil_tags.back().second < static_cast<int64_t>(rms_list.size())) {
                chunks.emplace_back(sil_tags.back().second * m_hopSize,
                                    static_cast<int64_t>(rms_list.size()) * m_hopSize);
            }
            return chunks;
        }
    }
