...
static std::vector<float> convertChannels(const std::vector<float> &output_buffer, size_t output_frames,
                                          size_t original_src_channels, size_t tar_channel) {
...
namespace AudioUtil
{
    SF_VIO resample_to_vio(const std::filesystem::path &filepath, std::string &msg, const int tar_channel,
        ...
        SF_VIO sf_vio_in;
        const std::string extension = dsfw::PathUtils::toUtf8(filepath.extension());
        ...
        if (extension == ".wav") {
            ...
            msg = "Unsupported file format: " + dsfw::PathUtils::toUtf8(filepath);
            return {};
        }
        ...
        soxr_t resampler = soxr_create(original_src_samplerate, tar_samplerate, original_src_channels, &error, &io_spec,
        ...
        while (!processing_error) {
            ...
            while (input_samples_processed < input_total_samples && !processing_error) {
                ...
                error = soxr_process(resampler, current_input_ptr, input_frames_this_call, &input_frames_done,
                                     output_buffer.data(), output_buffer_frames, &output_frames_done);
                ...
                if (output_frames_done > 0) {
                    ...
                    auto processed_buffer =
                        convertChannels(output_buffer, output_frames, original_src_channels, tar_channel);
        ...
        if (!processing_error) {
            size_t output_frames_done = 0;
            do {
                error = soxr_process(resampler, nullptr, 0, nullptr, output_buffer.data(), output_buffer_frames,
                                     &output_frames_done);
    ...
    bool readAudioToMonoFloat(const std::filesystem::path &filepath, std::vector<float> &samples,
                              int &sampleRate, std::string &msg) {
        SndfileHandle handle = openSndfile(filepath);
        if (!handle) {
            msg = "Failed to open audio file: " + dsfw::PathUtils::toUtf8(filepath) + " - " + sf_strerror(nullptr);
            return false;
        }

        sampleRate = handle.samplerate();
        const sf_count_t totalFrames = handle.frames();
        const int channels = handle.channels();

        std::vector<float> raw(static_cast<size_t>(totalFrames * channels));
        sf_count_t framesRead = handle.readf(raw.data(), totalFrames);
        if (framesRead != totalFrames) {
            msg = "Failed to read all frames from audio file";
            return false;
        }

        samples.resize(static_cast<size_t>(totalFrames));
        if (channels == 1) {
            ...
            for (sf_count_t i = 0; i < totalFrames; ++i) {
                ...
                samples[static_cast<size_t>(i)] = sum / static_cast<float>(channels);
            }
        }
