...
// Further, the header also has utility for a simple custom struct, where resources could be kept, to be registered as a
// custom op. For concrete examples, please search keyword "LiteCustomOpTest" under
// "<cloned_src_dir>/onnxruntime/test/". Note - all APIs in this header are ABI.
...
namespace Ort {
    namespace Custom {
        ...
        public:
            TensorBase(OrtKernelContext *ctx, size_t indice, bool is_input) : ArgBase(ctx, indice, is_input) {
            }
            ...
            Tensor(OrtKernelContext *ctx, size_t indice, bool is_input) : TensorBase(ctx, indice, is_input) {
                if (is_input_) {
                    ...
                    if (num_strings) {
                        ...
                        const_value.GetStringTensorContent(static_cast<void *>(chars.data()), num_chars, offsets.data(),
                                                           offsets.size());
        ...
        struct TensorArray : public ArgBase {
            ...
            Tensor<std::string> &AllocateStringTensor(size_t ith_output) {
                ...
                // indice_ + ith_output is the indice relative to context
                auto tensor =
                    std::make_unique<Tensor<std::string>>(ctx_.GetOrtKernelContext(), indice_ + ith_output, false);
        ...
        2. OrtLiteCustomFunc and OrtLiteCustomStruct, as two sub-structs, can be released in form of OrtLiteCustomOp
        since all members are kept in the OrtLiteCustomOp, hence memory could still be recycled properly. Further,
        OrtCustomOp is a c struct bearing no v-table, so offspring structs are by design to be of zero virtual functions
        to maintain cast safety.
        */
...
#define CREATE_TUPLE_INPUT(data_type)                                                                                  \
    template <size_t ith_input, size_t ith_output, typename T, typename... Ts>                                         \
    static                                                                                                             \
        typename std::enable_if<std::is_same<T, const Custom::Tensor<data_type> *>::value, std::tuple<T, Ts...>>::type \
        CreateTuple(OrtKernelContext *context, ArgPtrs &args, size_t num_input, size_t num_output,                     \
                    const std::string &ep) {                                                                           \
    ...
    template <size_t ith_input, size_t ith_output, typename T, typename... Ts>                                         \
    static typename std::enable_if<std::is_same<T, std::optional<const Custom::Tensor<data_type> *>>::value,           \
                                   std::tuple<T, Ts...>>::type                                                         \
        CreateTuple(OrtKernelContext *context, ArgPtrs &args, size_t num_input, size_t num_output,                     \
                    const std::string &ep) {                                                                           \
    ...
    template <size_t ith_input, size_t ith_output, typename T, typename... Ts>                                         \
    static                                                                                                             \
        typename std::enable_if<std::is_same<T, const Custom::Span<data_type> *>::value, std::tuple<T, Ts...>>::type   \
        CreateTuple(OrtKernelContext *context, ArgPtrs &args, size_t num_input, size_t num_output,                     \
                    const std::string &ep) {                                                                           \
        ...
        args.push_back(std::make_unique<Custom::Tensor<data_type>>(context, ith_input, true));                         \
        std::tuple<T> current =                                                                                        \
            std::tuple<T>{&reinterpret_cast<Custom::Tensor<data_type> *>(args.back().get())->AsSpan()};                \
        auto next = CreateTuple<ith_input + 1, ith_output, Ts...>(context, args, num_input, num_output, ep);           \
        return std::tuple_cat(current, next);                                                                          \
    }                                                                                                                  \
    template <size_t ith_input, size_t ith_output, typename T, typename... Ts>                                         \
    ...
    template <size_t ith_input, size_t ith_output, typename T, typename... Ts>                                         \
    static typename std::enable_if<std::is_same<T, std::optional<const Custom::Span<data_type> *>>::value,             \
                                   std::tuple<T, Ts...>>::type                                                         \
        CreateTuple(OrtKernelContext *context, ArgPtrs &args, size_t num_input, size_t num_output,                     \
                    const std::string &ep) {                                                                           \
        if (ith_input < num_input) {                                                                                   \
    ...
    template <size_t ith_input, size_t ith_output, typename T, typename... Ts>                                         \
    static typename std::enable_if<std::is_same<T, data_type>::value, std::tuple<T, Ts...>>::type CreateTuple(         \
        OrtKernelContext *context, ArgPtrs &args, size_t num_input, size_t num_output, const std::string &ep) {        \
        if ("CPUExecutionProvider" != ep) {                                                                            \
            ORT_CXX_API_THROW("scalar input could only be applied to CPU EP", OrtErrorCode::ORT_RUNTIME_EXCEPTION);    \
        }                                                                                                              \
        args.push_back(std::make_unique<Custom::Tensor<data_type>>(context, ith_input, true));                         \
        std::tuple<T> current =                                                                                        \
            std::tuple<T>{reinterpret_cast<Custom::Tensor<data_type> *>(args.back().get())->AsScalar()};               \
        auto next = CreateTuple<ith_input + 1, ith_output, Ts...>(context, args, num_input, num_output, ep);           \
        return std::tuple_cat(current, next);                                                                          \
    }                                                                                                                  \
    template <size_t ith_input, size_t ith_output, typename T, typename... Ts>                                         \
...
#define CREATE_TUPLE_OUTPUT(data_type)                                                                                 \
    ...
    template <size_t ith_input, size_t ith_output, typename T, typename... Ts>                                         \
    static typename std::enable_if<std::is_same<T, std::optional<Custom::Tensor<data_type> *>>::value,                 \
                                   std::tuple<T, Ts...>>::type                                                         \
        CreateTuple(OrtKernelContext *context, ArgPtrs &args, size_t num_input, size_t num_output,                     \
                    const std::string &ep) {                                                                           \
...
#define PARSE_INPUT_BASE(pack_type, onnx_type)                                                                         \
    template <typename T, typename... Ts>                                                                              \
    static typename std::enable_if<0 <= sizeof...(Ts) && std::is_same<T, pack_type>::value>::type ParseArgs(           \
        std::vector<ONNXTensorElementDataType> &input_types, std::vector<ONNXTensorElementDataType> &output_types) {   \
...
#define PARSE_ARGS(data_type, onnx_type)                                                                               \
    ...
    PARSE_OUTPUT(data_type, onnx_type)
            ...
            OrtLiteCustomOp(const char *op_name, const char *execution_provider, ShapeInferFn shape_infer_fn,
                            int start_ver = 1, int end_ver = MAX_CUSTOM_OP_END_VER) :
                op_name_(op_name), execution_provider_(execution_provider), shape_infer_fn_(shape_infer_fn),
                start_ver_(start_ver), end_ver_(end_ver) {
        ...
        //   "CPUExecutionProvider", Filter)}; v2_domain.Add(fil_op_ptr.get()); session_options.Add(v2_domain);
        ...
        struct OrtLiteCustomFunc : public OrtLiteCustomOp {
            ...
            OrtLiteCustomFunc(const char *op_name, const char *execution_provider, ComputeFn compute_fn,
                ...
                OrtCustomOp::KernelCompute = [](void *op_kernel, OrtKernelContext *context) {
                    ...
                    auto t =
                        CreateTuple<0, 0, Args...>(context, args, kernel->num_input_, kernel->num_output_, kernel->ep_);
                ...
                OrtCustomOp::KernelDestroy = [](void *op_kernel) { delete reinterpret_cast<Kernel *>(op_kernel); };
            ...
            OrtLiteCustomFunc(const char *op_name, const char *execution_provider,
                              ComputeFnReturnStatus compute_fn_return_status, ShapeInferFn shape_infer_fn = {},
                              int start_ver = 1, int end_ver = MAX_CUSTOM_OP_END_VER) :
                OrtLiteCustomOp(op_name, execution_provider, shape_infer_fn, start_ver, end_ver) {
        ...
        OrtLiteCustomOp *CreateLiteCustomOp(const char *op_name, const char *execution_provider,
                                            ...
                                            Status (*shape_infer_fn)(ShapeInferContext &) = {}, int start_ver = 1,
                                            int end_ver = MAX_CUSTOM_OP_END_VER) {
            using LiteOp = OrtLiteCustomFunc<Args...>;
            return std::make_unique<LiteOp>(op_name, execution_provider, custom_compute_fn, shape_infer_fn, start_ver,
                                            end_ver)
                .release();
        }
        ...
        OrtLiteCustomOp *CreateLiteCustomOp(const char *op_name, const char *execution_provider,
                                            ...
                                            int end_ver = MAX_CUSTOM_OP_END_VER) {
            using LiteOp = OrtLiteCustomFunc<Args...>;
            return std::make_unique<LiteOp>(op_name, execution_provider, custom_compute_fn_v2, shape_infer_fn,
                                            start_ver, end_ver)
                .release();
        }

        template <typename CustomOp>
        OrtLiteCustomOp *CreateLiteCustomOp(const char *op_name, const char *execution_provider, int start_ver = 1,
                                            int end_ver = MAX_CUSTOM_OP_END_VER) {
