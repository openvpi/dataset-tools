...
// To create an empty object, pass 'nullptr' to the constructor (for example, Env e{nullptr};). However, you can't use
// them until you assign an instance that actually holds an underlying object.
//
...
//
// ConstXXXX types are copyable since they do not own the underlying C object, so you can pass them to functions as
// arguments by value or by reference. ConstXXXX types are restricted to const only interfaces.
//
// UnownedXXXX are similar to ConstXXXX but also allow non-const interfaces.
//
// The lifetime of the corresponding owning object must eclipse the lifetimes of the ConstXXXX/UnownedXXXX types. They
// exists so you do not have to fallback to C types and the API with the usual pitfalls. In general, do not use C API
// from your C++ code.
...
#ifdef ORT_NO_EXCEPTIONS
// The #ifndef is for the very special case where the user of this library wants to define their own way of handling
// errors. NOTE: This header expects control flow to not continue after calling ORT_CXX_API_THROW
...
// initializers. Please define ORT_API_MANUAL_INIT if it conerns you.
...
#undef ORT_DEFINE_RELEASE
         ...
         *     ConstXXXX allow calling const interfaces only. They give access to objects that are owned by somebody
         * else such as Onnxruntime or instances of XXXX classes. d) serve convenient interfaces that return C++ objects
         * and further enhance exception and type safety so they can be used in C++ code.
         *
         */
    ...
    struct Status : detail::Base<OrtStatus> {
        ...
        Status(const char *message,
               OrtErrorCode code) noexcept; ///< Creates status instance out of null-terminated string message.
    ...
    struct RunOptions : detail::Base<OrtRunOptions> {
         ...
         *
         * If a currently executing session needs to be force terminated, this can be called from another thread to
         * force it to fail with an error Wraps OrtApi::RunOptionsSetTerminate
         */
        RunOptions &SetTerminate();

        /** \brief Clears the terminate flag so this RunOptions instance can be used in a new Session::Run call without
         * it instantly terminating
         *
    ...
    namespace detail {
        ...
        struct SessionOptionsImpl : ConstSessionOptionsImpl<T> {
            ...
            SessionOptionsImpl &SetGraphOptimizationLevel(
                GraphOptimizationLevel graph_optimization_level);    ///< Wraps OrtApi::SetSessionGraphOptimizationLevel
            ...
            SessionOptionsImpl &
                EnableProfiling(const ORTCHAR_T *profile_file_prefix); ///< Wraps OrtApi::EnableProfiling
            ...
            SessionOptionsImpl &
                SetExecutionMode(ExecutionMode execution_mode); ///< Wraps OrtApi::SetSessionExecutionMode
            ...
            SessionOptionsImpl &AddConfigEntry(const char *config_key,
            ...
            SessionOptionsImpl &
                AppendExecutionProvider(const std::string &provider_name,
                                        const std::unordered_map<std::string, std::string> &provider_options = {});
            ...
            SessionOptionsImpl &SetCustomJoinThreadFn(
                OrtCustomJoinThreadFn ort_custom_join_thread_fn); ///< Wraps OrtApi::SessionOptionsSetCustomJoinThreadFn

            ///< Registers the custom operator from the specified shared library via
            ///< OrtApi::RegisterCustomOpsLibrary_V2. The custom operator configurations are optional. If provided,
            ///< custom operator configs are set via OrtApi::AddSessionConfigEntry.
    ...
    struct ModelMetadata : detail::Base<OrtModelMetadata> {
         ...
         */
        AllocatedStringPtr
            GetProducerNameAllocated(OrtAllocator *allocator) const; ///< Wraps OrtApi::ModelMetadataGetProducerName
         ...
         */
        AllocatedStringPtr
            GetGraphNameAllocated(OrtAllocator *allocator) const; ///< Wraps OrtApi::ModelMetadataGetGraphName
         ...
         */
        AllocatedStringPtr
            GetDescriptionAllocated(OrtAllocator *allocator) const; ///< Wraps OrtApi::ModelMetadataGetDescription
        ...
        AllocatedStringPtr LookupCustomMetadataMapAllocated(
            const char *key, OrtAllocator *allocator) const; ///< Wraps OrtApi::ModelMetadataLookupCustomMetadataMap
    ...
    namespace detail {
        ...
        struct ConstSessionImpl : Base<T> {
            ...
            AllocatedStringPtr GetOverridableInitializerNameAllocated(
                size_t index, OrtAllocator *allocator) const; ///< Wraps OrtApi::SessionGetOverridableInitializerName
        ...
        struct SessionImpl : ConstSessionImpl<T> {
             ...
             * \param[in] input_names Array of null terminated strings of length input_count that is the list of input
             * names
             ...
             * \return A std::vector of Value objects that directly maps to the output_names array (eg. output_name[0]
             * is the first entry of the returned vector)
             */
             ...
             */
            void Run(const RunOptions &run_options, const char *const *input_names, const Value *input_values,
                     size_t input_count, const char *const *output_names, Value *output_values, size_t output_count);
             ...
             *             On calling RunAsync, output_values[i] could either be initialized by a null pointer or a
             * preallocated OrtValue*. Later, on invoking the callback, each output_values[i] of null will be filled
             * with an OrtValue* allocated by onnxruntime. Then, an OrtValue** pointer will be casted from
             * output_values, and pass to the callback. NOTE: it is customer's duty to finally release output_values and
             * each of its member, regardless of whether the member (Ort::Value) is allocated by onnxruntime or
             ...
             */
            void RunAsync(const RunOptions &run_options, const char *const *input_names, const Value *input_values,
                          size_t input_count, const char *const *output_names, Value *output_values,
                          size_t output_count, RunAsyncCallbackFn callback, void *user_data);
    ...
    namespace detail {
        ...
        struct TensorTypeAndShapeInfoImpl : Base<T> {
             ...
             */
            [[deprecated("use GetShape()")]] void
                GetDimensions(int64_t *values, size_t values_count) const; ///< Wraps OrtApi::GetDimensions

            void GetSymbolicDimensions(const char **values,
                                       size_t values_count) const; ///< Wraps OrtApi::GetSymbolicDimensions

            std::vector<int64_t>
                GetShape() const; ///< Uses GetDimensionsCount & GetDimensions to return a std::vector of the shape
        };
    ...
    namespace detail {
        ...
        struct ConstValueImpl : Base<T> {
            ...
            /// into a supplied buffer. Use GetStringTensorDataLength() to find out the length of the buffer to
            /// allocate. The user must also allocate offsets buffer with the number of entries equal to that of the
            /// contained strings.
            ///
        ...
        struct ValueImpl : ConstValueImpl<T> {
            ...
            /// The location of the indices is assumed to be the same as specified by OrtMemoryInfo argument at the
            /// creation time.
            /// </summary>
            /// <param name="indices_data">pointer to the user allocated buffer with indices. Use nullptr for fully
            /// sparse tensors.</param> <param name="indices_num">number of indices entries. Use 0 for fully sparse
            /// tensors</param>
            ...
            /// creation time.
            /// </summary>
            /// <param name="inner_data">pointer to the user allocated buffer with inner indices or nullptr for fully
            /// sparse tensors</param> <param name="inner_num">number of csr inner indices or 0 for fully sparse
            /// tensors</param> <param name="outer_data">pointer to the user allocated buffer with outer indices or
            /// nullptr for fully sparse tensors</param> <param name="outer_num">number of csr outer indices or 0 for
            /// fully sparse tensors</param>
            void UseCsrIndices(int64_t *inner_data, size_t inner_num, int64_t *outer_data, size_t outer_num);

            /// <summary>
            /// Supplies BlockSparse format specific indices and marks the contained sparse tensor as being a
            /// BlockSparse format tensor. Values are supplied with a CreateSparseTensor() API. The supplied indices are
            /// not copied and the user allocated buffers lifespan must eclipse that of the OrtValue. The location of
            /// the indices is assumed to be the same as specified by OrtMemoryInfo argument at the creation time.
            /// </summary>
            ...
            void FillSparseTensorBlockSparse(const OrtMemoryInfo *data_mem_info, const OrtSparseValuesParam &values,
    ...
    struct Value : detail::ValueImpl<OrtValue> {
         ...
         * OrtApi::CreateTensorAsOrtValue. This overload will allocate the buffer for the tensor  according to the
         * supplied shape and data type. The allocated buffer will be owned by the returned OrtValue and will be freed
         * when the OrtValue is released. The input data would need to be copied into the allocated buffer. This API is
         * not suitable for strings.
         *
         * \tparam T The numeric datatype. This API is not suitable for strings.
         ...
         *   The allocated buffer will be owned by the returned OrtValue and will be freed when the OrtValue is
         * released. The input data would need to be copied into the allocated buffer. This API is not suitable for
         * strings.
         *
         * \param allocator The allocator to use.
         * \param shape Pointer to the tensor shape dimensions.
        ...
        /// strings.
        /// </summary>
        /// <param name="allocator">allocator to use. The allocator lifespan must eclipse that of the resulting
        /// OrtValue</param> <param name="dense_shape">a would be dense shape of the tensor</param> <param
        /// name="type">data type</param> <returns>an instance of Ort::Value</returns>
        static Value CreateSparseTensor(OrtAllocator *allocator, const Shape &dense_shape,
                                        ONNXTensorElementDataType type);
    ...
    struct ArenaCfg : detail::Base<OrtArenaCfg> {
         ...
         * \param arena_extend_strategy -  use -1 to allow ORT to choose the default, 0 = kNextPowerOfTwo, 1 =
         * kSameAsRequested
...
#define ORT_CXX_LOGF_NOEXCEPT(logger, message_severity, /*format,*/...)                                                \
    ...
    struct Logger {
         ...
         * ORT_CXX_LOGF_NOEXCEPT macros to properly set the source code location and to use the cached severity level to
         * potentially bypass calls to the underlying C API. Returns an error status if a formatting error occurs.
         *
         * \param log_severity_level The message's logging severity level.
         * \param file_path The filepath of the file in which the message is logged. Usually the value of ORT_FILE.
         * \param line_number The file line number in which the message is logged. Usually the value of __LINE__.
    ...
    struct Op : detail::Base<OrtOp> {
        ...
        static Op Create(const OrtKernelInfo *info, const char *op_name, const char *domain, int version,
                         const char **type_constraint_names, const ONNXTensorElementDataType *type_constraint_values,
                         size_t type_constraint_count, const OpAttr *attr_values, size_t attr_count, size_t input_count,
                         size_t output_count);
        ...
        void Invoke(const OrtKernelContext *context, const OrtValue *const *input_values, size_t input_count,
                    OrtValue *const *output_values, size_t output_count);
    };
...
#    pragma warning(pop)
            ...
            if (WithStatus) {
#endif
                OrtCustomOp::CreateKernelV2 = [](const OrtCustomOp *this_, const OrtApi *api, const OrtKernelInfo *info,
