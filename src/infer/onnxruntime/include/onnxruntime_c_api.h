...
typedef struct OrtAllocator {
    ...
    const struct OrtMemoryInfo *(ORT_API_CALL *Info)(
        const struct OrtAllocator *this_); ///< Return a pointer to an ::OrtMemoryInfo that describes this allocator
} OrtAllocator;
...
typedef struct OrtROCMProviderOptions {
    ...
    OrtROCMProviderOptions() :
        ...
        tunable_op_enable{false}, tunable_op_tuning_enable{false}, tunable_op_max_tuning_duration_ms{} {
    }
#endif
...
struct OrtApi {
     ...
     * ::OrtTensorRTProviderOptionsV2 and value should be its related range.
     *
     ...
     * to know the available keys and values. Key should be in null terminated string format of the member of
     * ::OrtCUDAProviderOptionsV2 and value should be its related range.
     *
     ...
     * "sustained_high_performance". Default to "default". "qnn_saver_path": File path to the QNN Saver backend library.
     * If specified, QNN Saver will be enabled and will dump QNN API calls to disk for replay/debugging. QNN Saver
     * produces incorrect model inference results and may alter model/EP partitioning. Use only for debugging.
     *   "qnn_context_priority": QNN context priority, options: "low", "normal", "normal_high", "high". Default to
     * "normal". "htp_graph_finalization_optimization_mode": Set the optimization mode for graph finalization on the HTP
     ...
     * details. "soc_model": The SoC model number. Refer to the QNN SDK documentation for valid values. Defaults to "0"
     * (unknown). "htp_arch": The minimum HTP architecture the driver will use to select compatible QNN operators.
     * Available options:
     *     - "0": Default (none).
     *     - "68"
     *     - "69"
     ...
     *   "buffer_type": ITensor or user buffers, options: "ITENSOR", user buffer with different types - "TF8", "TF16",
     * "UINT8", "FLOAT". "ITENSOR" -- default, ITensor which is float only. "TF8" -- quantized model required, "FLOAT"
     * -- for both quantized or non-quantized model "enable_init_cache": enable SNPE init caching feature, set to 1 to
     * enabled it. Disabled by default. If SNPE is not available (due to a non Snpe enabled build or its dependencies
     * not being installed), this function will fail.
     *
     ...
     * \since Version 1.16.
     */
    ORT_API2_STATUS(KernelContext_GetResource, _In_ const OrtKernelContext *context, _In_ int resouce_version,
