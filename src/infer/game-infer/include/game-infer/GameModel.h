...
namespace Game
{
    ...
    private:
        ...
        std::vector<uint8_t> formatBoundaries(const std::vector<float> &durations, int64_t T,
        ...
        boundariesToDurations(const std::vector<uint8_t> &boundaries, const std::vector<uint8_t> &maskT) const;
        ...
        InferenceOutput inferSlice(const InferenceInput &input, float segThreshold, int segRadius, float estThreshold,
                                   const std::vector<float> &d3pmTs) const;
    };
} // namespace Game
