...
// ---------------------------------------------------------------------------
static void test_e2e(Game::Game &game, const std::filesystem::path &audioPath, const std::filesystem::path &outputDir,
                     float tempo) {
    ...
    // Save MIDI
    const auto midiPath = outputDir / (audioPath.stem().wstring() + L".mid");
