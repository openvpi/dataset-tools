...
namespace Rmvpe
{
    RmvpeModel::RmvpeModel(const std::filesystem::path &modelPath, const ExecutionProvider provider,
                           const int device_id) : CancellableOnnxModel(provider, device_id) {
    ...
    bool RmvpeModel::forward(const std::vector<float> &waveform_data, float threshold, std::vector<float> &f0,
        ...
        try {
            ...
            Ort::Value waveform_tensor =
                Ort::Value::CreateTensor<float>(m_memoryInfo, waveform_mutable.data(), waveform_mutable.size(),
            ...
            auto output_tensors = m_session->Run(runOptions, input_names, input_tensors, 2, output_names, 2);
            {
                std::lock_guard lock(m_runMutex);
