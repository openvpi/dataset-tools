...
namespace dstools {
    ...
    public:
        ...
        [[nodiscard]] Result<void> loadDictionary(const QString &dictPath);

        /// Load vowels from a file (one phoneme per line).
        [[nodiscard]] Result<void> loadVowelsFromFile(const QString &path);

        /// Load consonants from a file (one phoneme per line).
        [[nodiscard]] Result<void> loadConsonantsFromFile(const QString &path);
        ...
        [[nodiscard]] Result<QString> calculate(const QString &phSeq) const;

        /// Batch: fill phNum for each row.
        [[nodiscard]] Result<void> calculateBatch(std::vector<TranscriptionRow> &rows) const;
