...
namespace dstools {
    ...
    public:
        ...
        [[nodiscard]] static Result<std::vector<TranscriptionRow>> read(const QString &path);

        /// Write to file. Only writes columns that have non-empty values.
        [[nodiscard]] static Result<void> write(const QString &path, const std::vector<TranscriptionRow> &rows);

        /// Parse from in-memory string (for testing).
        [[nodiscard]] static Result<std::vector<TranscriptionRow>> parse(const QString &content);
    };
