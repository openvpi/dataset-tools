...
class PinyinG2PProvider : public dsfw::IG2PProvider {
public:
    ...
    dsfw::Result<std::vector<dsfw::G2PResult>> convert(const std::string& text, const std::string& langCode) override;

    /// @brief Convert a single word to its phoneme representation.
    /// @param word Input word.
    /// @param langCode Language code (e.g. "zh").
    /// @return A G2PResult or an error.
    dsfw::Result<dsfw::G2PResult> convertWord(const std::string& word, const std::string& langCode) override;
