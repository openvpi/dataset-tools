...
public:
    [[nodiscard]] static dsfw::Result<std::vector<double>> read(const QString& path);
    [[nodiscard]] static dsfw::Result<void> write(const QString& path, const std::vector<double>& times);
};
