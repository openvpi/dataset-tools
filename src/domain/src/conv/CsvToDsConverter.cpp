...
Result<std::vector<float>> extractF0Builtin(const QString& wavPath, int sampleRate, int hopSize, float minF0,
    ...
    const int64_t totalFrames = static_cast<int64_t>(mono.size());
    ...
    for (int frame = 0; frame < numFrames; ++frame) {
        ...
        if (bestCorr > 0.5f && bestLag > 0) {
            f0[static_cast<size_t>(frame)] = static_cast<float>(sampleRate) / static_cast<float>(bestLag);
        } else {
            f0[static_cast<size_t>(frame)] = 0.0f;
        }
    }
...
Result<void> CsvToDsConverter::convertFromMemory(const std::vector<TranscriptionRow>& rows, const Options& opts,
    ...
    for (int i = 0; i < total; ++i) {
        ...
        const QString wavPath = dsfw::PathUtils::fromStdPath(dsfw::PathUtils::join(
            dsfw::PathUtils::toStdPath(opts.wavsDir), (row.name + QStringLiteral(".wav")).toStdString()));
        const QString dsPath = dsfw::PathUtils::fromStdPath(dsfw::PathUtils::join(
            dsfw::PathUtils::toStdPath(opts.outputDir), (row.name + QStringLiteral(".ds")).toStdString()));
