...
dsfw::Result<void> ConfigMigration::migrate(nlohmann::json& json, int currentConfigVersion) const {
    for (int target = currentConfigVersion + 1;; ++target) {
        ...
        if (!it->second(json)) {
            return dsfw::Result<void>::Error("Config migration to version " + std::to_string(target) + " failed");
        }
    }
