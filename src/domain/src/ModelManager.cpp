...
static std::atomic<dsfw::ModelManager *> s_instance{nullptr};

dsfw::ModelManager &ModelManager::instance() {
...
ModelManager::ModelManager(QObject *parent) : QObject(parent) {
    ...
    if (prev) {
        qCritical() << "dsfw::ModelManager constructed more than once; previous instance at" << prev << "will be orphaned";
    }
    s_instance.store(this, std::memory_order_release);
}

ModelManager::~dsfw::ModelManager() {
    unloadAll();
    s_instance.store(nullptr, std::memory_order_release);
}

void ModelManager::registerProvider(ModelTypeId type, std::unique_ptr<dsfw::IModelProvider> provider) {
...
dsfw::IModelProvider *ModelManager::provider(ModelTypeId type) const {
    std::lock_guard lock(m_entriesMutex);
...
dsfw::Result<void> ModelManager::ensureLoaded(ModelTypeId type, const QString &modelPath, int gpuIndex) {
    if (QThread::currentThread() != QCoreApplication::instance()->thread()) {
        qCritical() << "ModelManager::ensureLoaded called from non-main thread";
        return dsfw::Result<void>::Error("ensureLoaded must be called from the main thread");
    }
    std::lock_guard lock(m_entriesMutex);
    auto it = m_entries.find(type);
    if (it == m_entries.end()) {
        return dsfw::Result<void>::Error("No provider registered for this model type");
    }
...
dsfw::Result<void> ModelManager::loadModel(const QString &taskKey, const TaskModelConfig &config, int gpuIndex) {
    if (config.modelPath.isEmpty())
        return dsfw::Result<void>::Error(("No model path configured for task: " + taskKey).toStdString());
