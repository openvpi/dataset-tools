...
void TestPipelineContext::buildTaskInput_ok() {
    ...
    ctx.layers[QStringLiteral("grapheme")] = LayerData::fromJson({
        {"text", "hello"}
    });
    ctx.layers[QStringLiteral("phoneme")] = LayerData::fromJson({
        {"phones", "h eh l ow"}
    });
...
void TestPipelineContext::json_roundtrip() {
    ...
    auto jStr = ctx.toJsonString();
    auto result = PipelineContext::fromJsonString(jStr);
    QVERIFY(result.ok());
