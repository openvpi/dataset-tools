...
void TestAppSettings::testGetDefault() {
    dsfw::AppSettings settings("TestApp_default");
    dsfw::SettingsKey<int> key("test/value", 42);
    QCOMPARE(settings.get(key), 42);
}

void TestAppSettings::testSetAndGet() {
    dsfw::AppSettings settings("TestApp_setget");
    dsfw::SettingsKey<int> key("test/count", 0);
    settings.set(key, 100);
    QCOMPARE(settings.get(key), 100);
}

void TestAppSettings::testSetBool() {
    dsfw::AppSettings settings("TestApp_bool");
    dsfw::SettingsKey<bool> key("feature/enabled", false);
    settings.set(key, true);
    QVERIFY(settings.get(key));
}

void TestAppSettings::testNestedPath() {
    dsfw::AppSettings settings("TestApp_nested");
    dsfw::SettingsKey<QString> key("ui/theme/name", QString("light"));
    settings.set(key, QString("dark"));
    QCOMPARE(settings.get(key), QString("dark"));
}

void TestAppSettings::testContains() {
    dsfw::AppSettings settings("TestApp_contains");
    dsfw::SettingsKey<int> key("some/key", 0);
...
void TestAppSettings::testRemove() {
    dsfw::AppSettings settings("TestApp_remove");
    dsfw::SettingsKey<int> key("del/me", 0);
    settings.set(key, 10);
    QVERIFY(settings.contains(key));
    settings.remove(key);
    QVERIFY(!settings.contains(key));
...
void TestAppSettings::testObserveNotify() {
    dsfw::AppSettings settings("TestApp_observe");
    dsfw::SettingsKey<int> key("obs/val", 0);
...
void TestAppSettings::testFlushReload() {
    ...
    dsfw::SettingsKey<int> key("persist/val", 0);

    {
        dsfw::AppSettings s1(appName);
        s1.set(key, 42);
        s1.flush();
    }
    {
        dsfw::AppSettings s2(appName);
