...
private:
    static DsPitchDocument makeDsPitchDocument(const QStringList &noteNames, int f0Frames = 100,
...
private slots:
    ...
    void testGetRestMidi_middleRest() {
        auto ds = makeDsPitchDocument({QStringLiteral("C4"), QStringLiteral("rest"),
                              QStringLiteral("E4")});
    ...
    void testGetRestMidi_allRest() {
        auto ds = makeDsPitchDocument({QStringLiteral("rest"), QStringLiteral("rest"),
                              QStringLiteral("rest")});
        double midi = PitchProcessor::getRestMidi(ds, 1);
        QCOMPARE(midi, 60.0);
    }
    ...
    void testGetRestMidi_firstNoteIsRest() {
        auto ds = makeDsPitchDocument({QStringLiteral("rest"), QStringLiteral("D4"),
                              QStringLiteral("E4")});
    ...
    void testApplyModulationDriftPreview_noOp() {
        auto ds = makeDsPitchDocument({QStringLiteral("C4")}, 50, 261.6);
        std::vector<float> pre = ds.f0.values;
        std::set<int> sel = {0};
    ...
    void testGetRestMidi_lastNoteIsRest() {
        auto ds = makeDsPitchDocument({QStringLiteral("C4"), QStringLiteral("E4"),
                              QStringLiteral("rest")});
    ...
    void testApplyModulationDriftPreview_restNoteSkipped() {
        auto ds = makeDsPitchDocument({QStringLiteral("C4"), QStringLiteral("rest")}, 100, 261.6);
        std::vector<float> pre = ds.f0.values;
        std::set<int> sel = {0, 1};
