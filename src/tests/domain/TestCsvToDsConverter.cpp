...
void TestCsvToDsConverter::testDsToCsv_basic() {
    ...
    QVERIFY(CsvToDsConverter::dsToCsv(tmp.path(), csvPath).ok());

    auto csvResult = TranscriptionCsv::read(csvPath);
    QVERIFY(csvResult.ok());
    QVERIFY(!csvResult->empty());
    QCOMPARE((*csvResult)[0].name, QString("test"));
    QCOMPARE((*csvResult)[0].phSeq, QString("h eh l ow"));
}

void TestCsvToDsConverter::testDsToCsv_nonExistentDir() {
    auto result = CsvToDsConverter::dsToCsv("/nonexistent/dir", "/tmp/out.csv");
    QVERIFY(!result.ok());
...
void TestCsvToDsConverter::testConvertFromMemory_withMockF0() {
    ...
    opts.hopSize = constants::kDefaultHopSize;

    auto result = CsvToDsConverter::convertFromMemory(rows, opts, mockF0, nullptr);
    QVERIFY(result.ok());
...
void TestCsvToDsConverter::testConvert_nonExistentCsv() {
    ...
    auto result = CsvToDsConverter::convert(opts, nullptr, nullptr);
    QVERIFY(!result.ok());
    QVERIFY(!result.error().empty());
}
...
void TestCsvToDsConverter::testConvertFromMemory_emptyRows() {
    ...
    QVERIFY(CsvToDsConverter::convertFromMemory(rows, opts, mockF0, nullptr).ok());
...
void TestCsvToDsConverter::testConvertFromMemory_dsContentVerification() {
    ...
    QCOMPARE(sv.phSeq, QString("a b"));
    QVERIFY(!sv.phDur.isEmpty());
    QVERIFY(!sv.f0Seq.isEmpty());
}
...
void TestCsvToDsConverter::testConvertFromMemory_f0CallbackError() {
    ...
    auto result = CsvToDsConverter::convertFromMemory(rows, opts, failingF0, nullptr);
    QVERIFY(!result.ok());
    QVERIFY(!result.error().empty());
}
...
void TestCsvToDsConverter::testDsToCsv_noteGlideFill() {
    ...
    QVERIFY(csvResult.ok());
    QVERIFY(!csvResult->empty());
    QVERIFY(!(*csvResult)[0].noteGlide.isEmpty());
}
