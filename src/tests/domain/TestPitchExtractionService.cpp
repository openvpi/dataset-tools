...
private slots:
    void testApplyPitchToDocumentAddsNewCurve() {
        ...
        std::vector<float> f0 = {440000.0f, 0.0f, 450000.0f};
    ...
    void testApplyPitchToDocumentReplacesExisting() {
        ...
        std::vector<float> f0 = {440000.0f, 450000.0f, 460000.0f};
        dstools::PitchExtractionService::applyPitchToDocument(doc, f0, 0.005f);
    ...
    void testApplyMidiToDocumentAddsNewLayer() {
        ...
        for (const auto &layer : doc.layers) {
            if (layer.name == QStringLiteral("midi")) {
                ...
                QCOMPARE(layer.boundaries.size(), 4u);
                QCOMPARE(layer.boundaries[0].text, QStringLiteral("C4"));
                QCOMPARE(layer.boundaries[1].text, QString());
                QCOMPARE(layer.boundaries[2].text, QStringLiteral("E4"));
                QCOMPARE(layer.boundaries[3].text, QString());
                break;
            }
        }
        QVERIFY(found);
