...
void TestDsProject::saveLoadRoundtrip() {
    ...
    QVERIFY(proj.saveFile(path).ok());

    auto loaded = DsProject::loadFile(path);
    QVERIFY(loaded.ok());
    QCOMPARE(loaded->workingDirectory(), dsfw::PathUtils::toNativeSeparators(QStringLiteral("/test/dir")));
...
void TestDsProject::v1Migration() {
    ...
    auto proj = DsProject::loadFile(path);
    QVERIFY(proj.ok());
...
#ifdef Q_OS_WIN
    QCOMPARE(dsfw::PathUtils::toPosixSeparators(QStringLiteral("C:\\Users\\test")), QStringLiteral("C:/Users/test"));
    QCOMPARE(dsfw::PathUtils::toNativeSeparators(QStringLiteral("C:/Users/test")), QStringLiteral("C:\\Users\\test"));
#else
    QCOMPARE(dsfw::PathUtils::toPosixSeparators(QStringLiteral("/usr/local/bin")), QStringLiteral("/usr/local/bin"));
    QCOMPARE(dsfw::PathUtils::toNativeSeparators(QStringLiteral("/usr/local/bin")), QStringLiteral("/usr/local/bin"));
#endif
    QCOMPARE(dsfw::PathUtils::toPosixSeparators(QStringLiteral("/usr/local/bin")), QStringLiteral("/usr/local/bin"));
}
...
void TestDsProject::slicerStateRoundtrip() {
    ...
    QCOMPARE(ls.slicePoints.size(), 1);
    QCOMPARE(ls.slicePoints.at(dsfw::PathUtils::toNativeSeparators(QStringLiteral("audio/a.wav"))).size(), 3);
}
