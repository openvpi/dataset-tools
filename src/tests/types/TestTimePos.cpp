...
void TestTimePos::secRoundtrip() {
    QCOMPARE(secToUs(1.2), dsfw::TimePos(1200000));
    QCOMPARE(usToSec(1200000), 1.2);

    // 6 decimal place round-trip
    QCOMPARE(secToUs(0.080000), dsfw::TimePos(80000));
    QCOMPARE(usToSec(80000), 0.08);
}

void TestTimePos::zero() {
    QCOMPARE(secToUs(0.0), dsfw::TimePos(0));
    QCOMPARE(usToSec(0), 0.0);
}

void TestTimePos::negative() {
    QCOMPARE(secToUs(-1.5), dsfw::TimePos(-1500000));
    QCOMPARE(usToSec(-1500000), -1.5);
}

void TestTimePos::largeValue() {
    // 1 hour — no overflow
    QCOMPARE(secToUs(3600.0), dsfw::TimePos(3600000000LL));
    QCOMPARE(usToSec(3600000000LL), 3600.0);
}

void TestTimePos::subUsRounding() {
    // 0.5 μs rounds to 1
    QCOMPARE(secToUs(0.0000005), dsfw::TimePos(1));
    // 0.4 μs rounds to 0
    QCOMPARE(secToUs(0.0000004), dsfw::TimePos(0));
}

void TestTimePos::msRoundtrip() {
    QCOMPARE(msToUs(1.0), dsfw::TimePos(1000));
    QCOMPARE(usToMs(1000), 1.0);

    QCOMPARE(msToUs(0.5), dsfw::TimePos(500));
...
void TestTimePos::constants() {
    QCOMPARE(kMicrosecondsPerSecond, dsfw::TimePos(1000000));
    QCOMPARE(kMicrosecondsPerMs, dsfw::TimePos(1000));
}
