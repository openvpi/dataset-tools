...
public:
    ...
    QStringList requiredInputFormats() const override { return {"ds"}; }
    ...
    bool exportItem(const QString &sourceFile, const QString & /*workingDir*/,
        ...
        for (int i = 0; i < doc.sentenceCount(); ++i) {
            ...
            // Parse ph_seq and ph_dur
            if (!s.contains("ph_seq") || !s.contains("ph_dur"))
                continue;

            QStringList phones =
                QString::fromStdString(s["ph_seq"].get<std::string>()).split(' ', Qt::SkipEmptyParts);
            QStringList durStrs =
                QString::fromStdString(s["ph_dur"].get<std::string>()).split(' ', Qt::SkipEmptyParts);

            if (phones.size() != durStrs.size())
                continue;

            double currentTime = offset; // in seconds
            for (int p = 0; p < phones.size(); ++p) {
                double dur = durStrs[p].toDouble();
                // Convert seconds to 100ns units (multiply by 10^7)
...
public:
    ...
    bool exportItem(const QString &sourceFile, const QString & /*workingDir*/,
        ...
        for (int i = 0; i < doc.sentenceCount(); ++i) {
            ...
            QStringList notes =
                QString::fromStdString(s["note_seq"].get<std::string>()).split(' ', Qt::SkipEmptyParts);
            QStringList noteDurs =
                QString::fromStdString(s["note_dur"].get<std::string>()).split(' ', Qt::SkipEmptyParts);

            // Optional text/lyrics
            QStringList lyrics;
            if (s.contains("text")) {
            ...
            for (int n = 0; n < notes.size() && n < noteDurs.size(); ++n) {
                ...
                int divisions = static_cast<int>(dur * 480.0); // quarter=1s approx
