...
// ─── Asr ──────────────────────────────────────────────────────────────────────
    ...
    Asr::Asr(const QString &modelPath, FunAsr::ExecutionProvider provider, int deviceId) {
        m_asrHandle = FunAsr::create_model(modelPath.toUtf8().toStdString().c_str(), 4, provider, deviceId);
...
// ─── AsrTask ──────────────────────────────────────────────────────────────────
    ...
    bool AsrThread::execute(QString &msg) {
        ...
        if (const auto asrRes = m_asr->recognize(dsfw::PathUtils::toStdPath(m_wavPath), asrMsg); !asrRes) {
        ...
        if (m_loader) {
            m_loader->writeLabFile(m_labPath, QString::fromStdString(asrMsg));
        }
        msg = QString::fromStdString(asrMsg);
        return true;
    }
...
bool FunAsrAdapter::load(const std::filesystem::path &modelPath,
    ...
    auto result = FunAsr::create_model(modelPath, 4, provider, deviceId);
    if (!result) {
        errorMsg = "Failed to load FunASR model from: " + dsfw::PathUtils::toUtf8(modelPath);
        return false;
    }
...
Result<void> FunAsrProcessor::initialize(ModelManager & /*mm*/,
    ...
    const int deviceId = static_cast<int>(configValueInt(modelConfig, QStringLiteral("deviceId"), -1));
