...
// ─── SmartHighlighter ─────────────────────────────────────────────────────────
    ...
    std::tuple<QString, QString, QString, int>
        ...
        for (int i = 0; i < static_cast<int>(aligned_asr.size()); ++i) {
...
// ─── LyricMatcher ─────────────────────────────────────────────────────────────
    ...
    LyricData LyricMatcher::process_lyric_file(const QString &lyric_path) const {
        ...
        file.close();
        return processLyricContent(content);
    }

    LyricData LyricMatcher::processLyricContent(const QString &content) const {
        const QString cleaned_text = m_processor.clean_text(content);
