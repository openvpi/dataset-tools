...
void DroppableFileListPanel::onAddFiles() {
    ...
    const QStringList files = dsfw::FileDialogHelper::getOpenFileNames({this, tr("Add Files"), {}, {filterStr}});
    if (!files.isEmpty())
        addFiles(files);
}

void DroppableFileListPanel::onAddDirectory() {
    const QString dir = dsfw::FileDialogHelper::getExistingDirectory({this, tr("Add Directory")});
    if (!dir.isEmpty())
        addDirectory(dir);
}
