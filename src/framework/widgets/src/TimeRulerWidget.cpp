...
QString TimeRulerWidget::formatTime(double timeSec, double intervalSec) {
    if (intervalSec >= 3600.0) {
        ...
        int sec = totalSec % 60;
        return QStringLiteral("%1:%2:%3").arg(hr).arg(min, 2, 10, QChar('0')).arg(sec, 2, 10, QChar('0'));
    }
    if (intervalSec >= 60.0) {
        int totalSec = static_cast<int>(timeSec);
        int min = totalSec / 60;
        int sec = totalSec % 60;
        return QStringLiteral("%1:%2").arg(min).arg(sec, 2, 10, QChar('0'));
    }
    if (intervalSec >= 1.0) {
...
void TimeRulerWidget::paintEvent(QPaintEvent* /*event*/) {
    ...
    auto level =
        findLevel(m_sampleRate > 0 && m_resolution > 0 ? static_cast<double>(m_sampleRate) / m_resolution : 200.0);
...
void TimeRulerWidget::wheelEvent(QWheelEvent* event) {
    ...
    double centerXRatio = static_cast<double>(event->position().x()) / width();
    emit rulerZoomRequested(event->angleDelta().y(), centerXRatio);
    event->accept();
}

void TimeRulerWidget::mousePressEvent(QMouseEvent* event) {
    if (event->button() == Qt::LeftButton && width() > 0) {
        double timeSec = m_viewStart + (static_cast<double>(event->pos().x()) / width()) * (m_viewEnd - m_viewStart);
        emit playheadMoveRequested(timeSec);
    }
