...
void PlayWidget::setPlaying(bool playing) {
    ...
    if (playing && !isPlaying()) {
        ...
        if (m_player->isOpen()) {
            if (m_hasRange) {
                m_lastObtainedTimeMs = static_cast<uint64_t>(m_rangeStart * 1000);
            } else {
...
void PlayWidget::registerPlayCallbacks(QObject *context,
                                       std::function<void()> playCb,
                                       std::function<void()> stopCb) {
    connect(this, &PlayWidget::playRequested, context, playCb);
    connect(this, &PlayWidget::playStopped, context, stopCb);
}
