...
namespace dsfw::signal {
    ...
    std::vector<float> resampleCurve(const std::vector<float> &values, dstools::TimePos srcTimestep,
                                     dstools::TimePos dstTimestep, int alignLength) {
    ...
    void interpUnvoiced(std::vector<int32_t> &values, std::vector<bool> *uvOut) {
        ...
        for (int i = first; i <= last; ++i) {
            ...
            const double logPrev = std::log2(dstools::mhzToHz(values[prev]));
            const double logNext = std::log2(dstools::mhzToHz(values[next]));
            ...
            for (int j = i; j < next; ++j) {
                ...
                values[j] = dstools::hzToMhz(std::pow(2.0, logInterp));
            }
            i = next - 1;
        }
    }
    ...
    std::vector<int32_t> hzToMhzBatch(const std::vector<double> &hz) {
        ...
        for (size_t i = 0; i < hz.size(); ++i)
            out[i] = (hz[i] == 0.0) ? 0 : dstools::hzToMhz(hz[i]);
        return out;
    }
    ...
    std::vector<double> mhzToMidiBatch(const std::vector<int32_t> &mhz) {
        std::vector<double> out(mhz.size());
        for (size_t i = 0; i < mhz.size(); ++i) {
            const double hz = dstools::mhzToHz(mhz[i]);
    ...
    void dragBoundary(std::vector<double> &values, int boundaryIdx, double delta, int influenceRadius) {
        ...
        const double sigma = influenceRadius / 3.0; // 3σ covers most of the influence
