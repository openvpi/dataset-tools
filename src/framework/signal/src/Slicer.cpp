...
#ifdef DSFW_SIGNAL_ENABLE_XSIMD
...
namespace dsfw::signal
{
    ...
    static inline int64_t argmin(const Iterator &begin, const Iterator &end) {
        if (begin == end) {
            std::fprintf(stderr, "[Slicer] argmin: empty iterator range\n");
            return 0;
        }
    ...
    static inline std::vector<double> get_rms_impl_basic(const std::vector<float> &samples, const int frame_length,
        ...
        for (size_t i = 0; i < output_size; ++i) {
            const bool is_underflow = i * hop_length < static_cast<size_t>(frame_length / 2);
    ...
    Slicer::Slicer(int sampleRate, float threshold, int hopSize, int winSize, int minLength, int minInterval,
                   int maxSilKept) :
        m_sampleRate(sampleRate), m_threshold(threshold), m_hopSize(hopSize), m_winSize(winSize),
        m_minLength(minLength), m_minInterval(minInterval), m_maxSilKept(maxSilKept) {}
    ...
    Slicer Slicer::fromMilliseconds(int sampleRate, const SlicerParams &params) {
        ...
        int hopSize = static_cast<int>(divIntRound<int64_t>(static_cast<int64_t>(params.hopSize) * sampleRate, 1000LL));
        int winSize = static_cast<int>(
            std::min(divIntRound<int64_t>(static_cast<int64_t>(params.minInterval) * sampleRate, 1000LL),
                     static_cast<int64_t>(4) * hopSize));
        int minLength = static_cast<int>(
            divIntRound<int64_t>(static_cast<int64_t>(params.minLength) * sampleRate, 1000LL * hopSize));
        int minInterval = static_cast<int>(
            divIntRound<int64_t>(static_cast<int64_t>(params.minInterval) * sampleRate, 1000LL * hopSize));
        int maxSilKept = static_cast<int>(
            divIntRound<int64_t>(static_cast<int64_t>(params.maxSilKept) * sampleRate, 1000LL * hopSize));

        return Slicer(sampleRate, threshold, hopSize, winSize, minLength, minInterval, maxSilKept);
    }
    ...
    MarkerList Slicer::slice(const std::vector<float> &samples) const {
        ...
        if (silence_start >= 0 && static_cast<int64_t>(rms_list.size()) - silence_start >= m_minInterval) {
            const int64_t silence_end = (std::min)(static_cast<int64_t>(rms_list.size() - 1),
                                                   silence_start + static_cast<int64_t>(m_maxSilKept));
            int64_t pos = argmin(rms_list.begin() + silence_start, rms_list.begin() + silence_end + 1);
            pos += silence_start;
            sil_tags.emplace_back(pos, static_cast<int64_t>(rms_list.size()) + 1);
        }
