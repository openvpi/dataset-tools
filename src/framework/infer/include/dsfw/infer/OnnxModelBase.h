...
namespace dsfw::infer {
    ...
    protected:
        ...
        OnnxModelBase(ExecutionProvider provider, int deviceId) : m_provider(provider), m_deviceId(deviceId) {
        }

        [[nodiscard]] Result<void> loadSession(const std::filesystem::path &modelPath);
        [[nodiscard]] Result<void> loadSession(const std::filesystem::path &modelPath, ExecutionProvider provider, int deviceId);

        [[nodiscard]] static Result<void> loadSessionTo(std::unique_ptr<Ort::Session> &target, const std::filesystem::path &modelPath,
                                          ExecutionProvider provider, int deviceId);

        [[nodiscard]] Result<nlohmann::json> loadConfig(const std::filesystem::path &modelDir);
    ...
    public:
        [[nodiscard]] static Result<void> validateModelFile(const std::filesystem::path &modelPath);
        virtual ~OnnxModelBase() noexcept = default;
    ...
    protected:
        ...
        CancellableOnnxModel(ExecutionProvider provider, int deviceId) : OnnxModelBase(provider, deviceId) {
        }
