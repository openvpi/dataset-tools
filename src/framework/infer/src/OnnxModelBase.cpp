...
namespace dstools::infer {
    ...
    Result<void> OnnxModelBase::validateModelFile(const std::filesystem::path &modelPath) {
        ...
        if (!std::filesystem::exists(modelPath, ec) || ec) {
            return Err("Model file does not exist: " + PathUtils::toUtf8(modelPath));
        }

        const auto fileSize = std::filesystem::file_size(modelPath, ec);
        if (ec) {
            return Err("Cannot read model file size: " + PathUtils::toUtf8(modelPath));
        }
        if (fileSize == 0) {
            return Err("Model file is empty: " + PathUtils::toUtf8(modelPath));
        }
        if (fileSize < 16) {
            return Err("Model file is too small to be a valid ONNX model (" +
                       std::to_string(fileSize) + " bytes): " + PathUtils::toUtf8(modelPath));
        }

        auto file = PathUtils::openIfstream(modelPath, std::ios::binary);
        if (!file.is_open()) {
            return Err("Cannot open model file for reading: " + PathUtils::toUtf8(modelPath));
        }

        char header[4] = {};
        if (!file.read(header, 4)) {
            return Err("Cannot read model file header: " + PathUtils::toUtf8(modelPath));
        }
    ...
    Result<nlohmann::json> OnnxModelBase::loadConfig(const std::filesystem::path &modelDir) {
        ...
        auto file = PathUtils::openIfstream(configPath);
        if (!file.is_open()) {
            return Err<nlohmann::json>("Cannot open config.json in " + PathUtils::toUtf8(modelDir));
        }
        try {
