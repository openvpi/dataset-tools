...
Ort::SessionOptions OnnxEnv::createSessionOptions(ExecutionProvider provider, [[maybe_unused]] int deviceId,
                                                  int interOpThreads) {
    ...
    switch (provider) {
        ...
        case ExecutionProvider::DML: {
            ...
            if (getApiStatus.IsOK() && ortDmlApi) {
                ...
                if (!appendStatus.IsOK()) {
                    std::cerr << "Warning: Failed to enable DML execution provider: " << appendStatus.GetErrorMessage()
                              << std::endl;
        ...
        case ExecutionProvider::CUDA: {
            ...
            if (createStatus.IsOK()) {
                ...
                std::cerr << "[infer] CreateCUDAProviderOptions failed: " << createStatus.GetErrorMessage()
                          << std::endl;
            }
            break;
        }
