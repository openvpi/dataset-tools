...
namespace dstools {
    ...
    /// Handles: font setup, root privilege check, crash handler, crash recovery detection.
    ///
    ...
    struct AppInit {
        ...
        /// @brief Whether the previous application session ended abnormally (crash).
        /// Must be called after init(). Internally delegates to CrashSafeGuard.
