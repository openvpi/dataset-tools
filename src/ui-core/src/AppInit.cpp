...
bool AppInit::init(QApplication &app, bool initCrashHandler) {
    ...
    // 4.6 Crash recovery detection: check if previous session ended abnormally
    dsfw::CrashSafeGuard::markStartup(logDir);
...
bool AppInit::wasPreviousCrash() {
    return dsfw::CrashSafeGuard::wasPreviousCrash();
}
