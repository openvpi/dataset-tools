...
void GameAlignPage::buildUi() {
    ...
    connect(m_runProgress, &dsfw::widgets::RunProgressRow::runClicked, this, [this]() {
        ...
        const QString audioDir = dsfw::PathUtils::fromStdPath(
            dsfw::PathUtils::join(dsfw::PathUtils::toStdPath(m_workingDir), "dstemp/slicer_output"));
        const QString csvPath = dsfw::PathUtils::fromStdPath(
            dsfw::PathUtils::join(dsfw::PathUtils::toStdPath(m_workingDir), "dstemp/build_csv/transcriptions.csv"));
        ...
        opts.snapshotDir = ProjectPaths::slicesDir(m_workingDir) + QStringLiteral("/game_align_snapshots");
