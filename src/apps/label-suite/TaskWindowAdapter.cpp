...
void TaskWindowAdapter::connectPipelineRunner(dsfw::PipelineRunner *runner) {
    ...
    QObject::connect(runner, &dsfw::PipelineRunner::stepCompleted,
                     this, [this](int, const QString &name) {
                         m_page->appendLog(QStringLiteral("Step completed: %1").arg(name));
                     });
    QObject::connect(runner, &dsfw::PipelineRunner::itemDiscarded,
