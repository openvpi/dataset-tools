...
namespace dstools::labeler {
    ...
    static const char *stepDirs[] = {
        "slicer", "asr", "asr_review", "alignment", "alignment_review", "build_csv", "midi", "build_ds", "pitch_review",
    };
    ...
    void CleanupDialog::buildLayout() {
        ...
        auto *desc = new QLabel(tr("Select intermediate files to delete. Original audio files are never deleted."));
    ...
    void CleanupDialog::scanDirectory() {
        QDir base(dsfw::PathUtils::fromStdPath(
            dsfw::PathUtils::join(dsfw::PathUtils::toStdPath(m_workingDir), "dstemp")));
        for (int i = 0; i < 9; ++i) {
            QDir stepDir(base.filePath(stepDirs[i]));
            if (!stepDir.exists()) {
                m_checkBoxes[i]->setText(tr("Step %1 (%2): no files").arg(i + 1).arg(stepNames[i]));
            ...
            m_checkBoxes[i]->setText(
                tr("Step %1 (%2): %3 files, %4").arg(i + 1).arg(stepNames[i]).arg(fileCount).arg(sizeStr));
