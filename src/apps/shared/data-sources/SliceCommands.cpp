...
namespace dstools {
    ...
    AddSlicePointCommand::AddSlicePointCommand(std::vector<double> &points, double timeSec,
        ...
        QUndoCommand(QObject::tr("Add slice point")), m_points(points), m_time(timeSec), m_refresh(std::move(refresh)) {
        m_insertIndex = static_cast<int>(std::lower_bound(m_points.begin(), m_points.end(), m_time) - m_points.begin());
    }
    ...
    MoveSlicePointCommand::MoveSlicePointCommand(std::vector<double> &points, int index, double oldTime, double newTime,
                                                 std::function<void()> refresh) :
        QUndoCommand(QObject::tr("Move slice point")), m_points(points), m_index(index), m_oldTime(oldTime),
        m_newTime(newTime), m_refresh(std::move(refresh)) {
    }
    ...
    RemoveSlicePointCommand::RemoveSlicePointCommand(std::vector<double> &points, int index,
                                                     std::function<void()> refresh) :
        QUndoCommand(QObject::tr("Remove slice point")), m_points(points), m_index(index), m_time(points[index]),
