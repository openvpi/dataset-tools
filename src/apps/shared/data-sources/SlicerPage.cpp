...
void SlicerPage::buildLayout() {
...

    {
        ...
        m_btnToggleSidebar->setText(QStringLiteral("\u25C0"));
        m_btnToggleSidebar->setToolTip(QStringLiteral("折叠/展开右侧面板"));
        m_btnToggleSidebar->setCheckable(true);
        m_btnToggleSidebar->setStyleSheet(
            "QToolButton { padding: 2px 8px; border-radius: 3px; font-size: 14px; color: #9898A8; }"
            "QToolButton:hover { background-color: #3A3A48; color: #C0C0D0; }"
            "QToolButton:checked { background-color: #4A90D9; color: white; }");
        m_toolbar->addWidget(m_btnToggleSidebar);
    }
    ...
    setupContainerAndPlayWidget(5000);
    ...
    m_waveformChart->setBoundaryModel(m_boundaryModel.get());

    addMouthCurveChart(1, 1, 0.5);

    addSpectrogramChart(1, 1, 5.0);
    m_spectrogramChart->setBoundaryModel(m_boundaryModel.get());
    m_spectrogramChart->setVisible(true);

    m_container->setBoundaryModel(m_boundaryModel.get());

    connect(m_container, &AudioVisualizerContainer::boundaryModelInvalidated, this, [this]() {
        m_slicePoints = m_boundaryModel->slicePointsSec();
        updateSlicerListPanel();
    });

    auto onDragging = [this]() {
        m_slicePoints = m_boundaryModel->slicePointsSec();
        updateSlicerListPanel();
    };
    ...
    m_contentSplitter->addWidget(m_container);
    m_contentSplitter->addWidget(m_sliceListPanel);
    m_contentSplitter->setStretchFactor(0, 1);
    m_contentSplitter->setStretchFactor(1, 0);
    m_contentSplitter->setCollapsible(1, true);
    m_sliceListPanel->setMinimumWidth(180);
    m_sliceListPanel->setMaximumWidth(320);
    ...
    connect(m_btnToggleSidebar, &QToolButton::toggled, this, [this](bool checked) {
        ...
        if (checked) {
            ...
            if (!m_sidebarExpandedSize.isEmpty())
                m_contentSplitter->setSizes(m_sidebarExpandedSize);
            else
                m_contentSplitter->setSizes({3, 1});
        }
    });
}
...
void SlicerPage::onImportMarkers() {
    ...
    auto result = AudacityMarkerAdapter::read(path);
    if (!result.ok()) {
        QMessageBox::warning(this, tr("Import Error"), QString::fromStdString(result.error()));
        return;
    }
...
void SlicerPage::onSaveMarkers() {
    ...
    auto result = AudacityMarkerAdapter::write(path, m_slicePoints);
    if (!result.ok())
        QMessageBox::warning(this, tr("Save Error"), QString::fromStdString(result.error()));
}
...
void SlicerPage::onExportAudio() {
    ...
    QString msg = tr("Exported %1 slice files to:\n%2").arg(exported).arg(outputDir);
    if (skipped > 0)
        msg += tr("\n\n%1 discarded slices were skipped.").arg(skipped);
    if (overlapped > 0)
        msg += tr("\n\n%1 overlapped slices were skipped.").arg(overlapped);
    QMessageBox::information(this, tr("Export Complete"), msg);
}
...
void SlicerPage::onActivated() {
    ...
    m_container->setResolutionKey(QStringLiteral("Viewport/slicer/resolution"));
