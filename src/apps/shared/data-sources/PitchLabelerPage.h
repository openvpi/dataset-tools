...
private:
    ...
    std::shared_ptr<Rmvpe::Rmvpe> m_rmvpe;
    std::shared_ptr<Game::Game> m_game;
    PhNumCalculator m_phNumCalc;

    Rmvpe::Rmvpe* rmvpe() const { return m_rmvpe.get(); }
    Game::Game* game() const { return m_game.get(); }
...
public:
    /// @brief Acquire inference engines from the engine pool.
    void acquireEngines();

    QAction* extractPitchAction() const { return m_extractPitchAction; }
    QAction* extractMidiAction() const { return m_extractMidiAction; }
    void loadPhNumCalculator();
    PhNumCalculator phNumCalc() const { return m_phNumCalc; }
