...
namespace dstools {
    ...
    PitchLabelerPage::PitchLabelerPage(QWidget *parent) : EditorPageBase("PitchLabeler", parent) {
        ...
        shortcutManager()->bind(m_editor->zoomResetAction(), dstools::settings::kShortcutZoomReset, tr("Reset Zoom"), tr("View"));
        shortcutManager()->bind(m_editor->abCompareAction(), dstools::settings::kShortcutABCompare, tr("A/B Compare"), tr("Tools"));
        shortcutManager()->bind(m_extractPitchAction, dstools::settings::kShortcutExtractPitch, tr("Extract Pitch"), tr("Processing"));
        shortcutManager()->bind(m_extractMidiAction, dstools::settings::kShortcutExtractMidi, tr("Extract MIDI"), tr("Processing"));
        shortcutManager()->bind(m_editor->playPauseAction(), dstools::settings::kShortcutPlayPause, tr("Play/Pause"), tr("Playback"));
    ...
    bool PitchLabelerPage::saveCurrentSlice() {
        ...
        DsTextDocBridge::fromPitchDoc(doc, *m_currentFile);
    ...
    QMenuBar *PitchLabelerPage::createMenuBar(QWidget *parent) {
        ...
        auto *viewMenu = addStandardViewMenu(bar, {m_editor->zoomInAction(), m_editor->zoomOutAction(), m_editor->zoomResetAction()});
    ...
    void PitchLabelerPage::onAutoInferProcessDirty(const QStringList &dirty) {
        ...
        if (needPhNum) {
            ...
            if (m_phNumCalc.isLoaded()) {
                dsfw::widgets::ToastNotification::show(this, dsfw::widgets::ToastType::Info, tr("自动计算发音数..."),
                                                       3000);
    ...
    bool PitchLabelerPage::buildAlignInput(Game::AlignInput &outAlignInput) const {
        ...
        for (const auto &layer : doc.layers) {
            if (layer.name.contains(QString::fromUtf8(dstools::keys::layers::phoneme), Qt::CaseInsensitive) && !layer.boundaries.empty()) {
                ...
                if (result) {
                    ...
                    DSFW_LOG_INFO("infer", ("Pitch extraction completed: " + sliceId.toStdString() + " - " +
                                            std::to_string(result.value().f0Mhz.size()) + " frames")
                                               .c_str());
                    dsfw::widgets::ToastNotification::show(this, dsfw::widgets::ToastType::Info, tr("音高提取完成"),
                                                           3000);
                } else {
                ...
                if (result) {
                    ...
                    DSFW_LOG_INFO("infer", ("MIDI transcription completed: " + sliceId.toStdString() + " - " +
                                            std::to_string(result.value().size()) + " notes")
                                               .c_str());
                    dsfw::widgets::ToastNotification::show(this, dsfw::widgets::ToastType::Info, tr("MIDI 转录完成"),
                                                           3000);
                } else {
    ...
    void PitchLabelerPage::runAddPhNum(const QString &sliceId) {
        ...
        if (calcResult.ok()) {
            ...
            if (auto res = source()->saveSlice(sliceId, doc); !res.ok())
                DSFW_LOG_ERROR(
                    "pitch",
                    ("Failed to save slice after ph_num: " + sliceId.toStdString() + " - " + res.error()).c_str());
        } else {
            QMessageBox::warning(this, tr("计算 ph_num"), tr("ph_num 计算失败: %1").arg(QString::fromStdString(calcResult.error())));
        }
    }
