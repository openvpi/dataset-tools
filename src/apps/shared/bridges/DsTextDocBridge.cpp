...
namespace dstools {
    ...
    void DsTextDocBridge::fromPitchDoc(DsTextDocument &doc, const pitchlabeler::DsPitchDocument &pdoc) {
    ...
    Result<void> DsTextDocBridge::verifyLayerRoundtrip(const DsTextDocument &original,
                                                        const DsTextDocument &restored) {
        if (original.layers.size() != restored.layers.size())
            return Result<void>::Error("Layer count mismatch: " + std::to_string(original.layers.size())
                                       + " vs " + std::to_string(restored.layers.size()));
        ...
        for (size_t li = 0; li < original.layers.size(); ++li) {
            ...
            if (orig.boundaries.size() != rest.boundaries.size())
                return Result<void>::Error("Boundary count mismatch in layer '" + orig.name.toStdString()
                                           + "': " + std::to_string(orig.boundaries.size())
                                           + " vs " + std::to_string(rest.boundaries.size()));
        ...
        for (const auto &curve : original.curves) {
            if (curve.name == QString::fromUtf8(dstools::keys::layers::pitch)) {
                if (curve.values.size() != restored.f0.values.size())
                    return Result<void>::Error("Pitch curve value count mismatch: "
                                               + std::to_string(curve.values.size())
        ...
        for (const auto &layer : original.layers) {
            if (layer.name == QString::fromUtf8(dstools::keys::layers::midi)) {
                if (layer.boundaries.size() != restored.notes.size() * 2)
                    return Result<void>::Error("MIDI boundary count mismatch: "
                                               + std::to_string(layer.boundaries.size())
