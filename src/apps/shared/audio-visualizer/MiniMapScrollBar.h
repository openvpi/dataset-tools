...
namespace dstools {
    ...
    public:
        explicit MiniMapScrollBar(dsfw::widgets::ViewportController *viewport, QWidget *parent = nullptr);
        ~MiniMapScrollBar() override;

        void setAudioData(const std::vector<float> &samples, int sampleRate);
        void setTotalDuration(double durationSec);
    ...
    private:
        ...
        [[nodiscard]] double timeToX(double sec) const;
