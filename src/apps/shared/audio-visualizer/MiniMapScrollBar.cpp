...
void MiniMapScrollBar::setAudioData(const std::vector<float> &samples, int sampleRate) {
    m_samples = samples;
    m_sampleRate = sampleRate > 0 ? sampleRate : constants::kDefaultSampleRate;
...
void MiniMapScrollBar::setTotalDuration(double durationSec) {
    m_totalDuration = durationSec;
    rebuildPeakCache();
    update();
}
...
double MiniMapScrollBar::timeToX(double sec) const {
    if (m_totalDuration <= 0.0 || width() <= 0) return 0.0;
    return std::round(sec / m_totalDuration * width());
}
...
void MiniMapScrollBar::paintEvent(QPaintEvent * /*event*/) {
    ...
    if (m_totalDuration > 0.0) {
        int x1 = static_cast<int>(timeToX(m_viewStart));
        int x2 = static_cast<int>(timeToX(m_viewEnd));
