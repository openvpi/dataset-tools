...
AudioVisualizerContainer::AudioVisualizerContainer(const QString& settingsAppName, QWidget* parent)
    ...
    connect(m_timeRuler, &TimeRulerWidget::rulerZoomRequested, this, [this](double angleDelta, double centerXRatio) {
        if (!m_viewport)
            return;
        double centerSec = m_viewport->startSec() + centerXRatio * m_viewport->duration();
        if (angleDelta > 0) {
            if (m_viewport->canZoomIn())
                m_viewport->zoomIn(centerSec);
        } else {
            if (m_viewport->canZoomOut())
                m_viewport->zoomOut(centerSec);
        }
    });

    connect(m_timeRuler, &TimeRulerWidget::playheadMoveRequested, this, [this](double timeSec) {
        if (m_playWidget)
    ...
    connect(m_viewport, &ViewportController::viewportChanged, this, &AudioVisualizerContainer::updateScaleIndicator);
...
void AudioVisualizerContainer::setYAxisWidth(int w) {
    ...
    if (m_tierRadioPanel)
        m_tierRadioPanel->setFixedWidth(w);
}
...
void AudioVisualizerContainer::setTierRadioPanel(QWidget *panel) {
    if (m_tierRadioPanel) {
        m_tierRadioPanel->deleteLater();
    }
    m_tierRadioPanel = panel;
    if (panel) {
        panel->setParent(m_leftPane);
        panel->show();
        updateTierRadioPanelGeometry();
    }
}

void AudioVisualizerContainer::updateTierRadioPanelGeometry() {
    if (!m_tierRadioPanel || !m_miniMap || !m_timeRuler || !m_editorWidget)
        return;
    int y = m_miniMap->height() + m_timeRuler->height();
    int h = m_editorWidget->height();
    m_tierRadioPanel->move(0, y);
    m_tierRadioPanel->setFixedHeight(std::max(1, h));
    m_tierRadioPanel->setFixedWidth(m_leftPane->width());
}
