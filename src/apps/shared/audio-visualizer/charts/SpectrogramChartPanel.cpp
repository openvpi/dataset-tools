...
void SpectrogramChartPanel::setAudioData(const std::vector<float> &samples, int sampleRate) {
    m_samples = samples;
...
void SpectrogramChartPanel::drawContent(QPainter &painter) {
    if (m_samples.empty()) {
...
void SpectrogramChartPanel::rebuildCache() {
    rebuildViewImage();
}
...
void SpectrogramChartPanel::rebuildViewImage() {
    ...
    if (m_viewImage.width() == w && m_viewImage.height() == h &&
        m_cachedViewStart == m_xf.viewStart && m_cachedViewEnd == m_xf.viewEnd) {
        return;
    }
    ...
    double viewFrameStartF = (m_xf.viewStart / totalDuration) * totalFrames;
    double viewFrameEndF = (m_xf.viewEnd / totalDuration) * totalFrames;
    int margin = 2;
    ...
    for (int x = 0; x < w; ++x) {
        double t = m_xf.viewStart + (m_xf.viewEnd - m_xf.viewStart) * (x + 0.5) / w;
    ...
    m_cachedViewStart = m_xf.viewStart;
    m_cachedViewEnd = m_xf.viewEnd;
    m_cachedWidth = w;
    m_cachedHeight = h;
}
