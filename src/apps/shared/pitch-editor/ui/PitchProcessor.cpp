...
namespace dstools {
    namespace pitchlabeler {
        namespace ui {
            ...
            void PitchProcessor::applyModulationDriftPreview(DsPitchDocument &ds, const std::vector<float> &preAdjustF0,
                                                             const std::set<int> &selectedNotes,
                                                             double modulationAmount, double driftAmount) {
                auto &f0 = ds.f0;
                if (f0.timestep <= 0 || preAdjustF0.empty())
                    return;
                ...
                for (int noteIdx : selectedNotes) {
                    ...
                    int endIdx =
                        std::min(static_cast<int>(f0.values.size()),
                                 static_cast<int>(std::ceil(static_cast<double>(note.end() - offset) / f0.timestep)));
                    if (startIdx >= endIdx)
                        continue;
                    ...
                    for (int i = 0; i < n; ++i) {
                        ...
                        newVals[i] = std::clamp(newCenter + newMod, midiToFreq(24.0), midiToFreq(108.0));
                    }
