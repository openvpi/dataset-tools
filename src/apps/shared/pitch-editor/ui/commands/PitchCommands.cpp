...
namespace dstools {
    namespace pitchlabeler {
        namespace ui {
            ...
            PitchMoveCommand::PitchMoveCommand(std::shared_ptr<DsPitchDocument> dsFile, std::vector<int> indices, int deltaCents,
                                               QUndoCommand *parent) :
            ...
            ModulationDriftCommand::ModulationDriftCommand(std::shared_ptr<DsPitchDocument> dsFile,
                                                           std::vector<float> oldF0Values,
                                                           std::vector<float> newF0Values, QUndoCommand *parent) :
                QUndoCommand(parent), m_dsFile(std::move(dsFile)), m_oldF0(std::move(oldF0Values)),
                m_newF0(std::move(newF0Values)) {
