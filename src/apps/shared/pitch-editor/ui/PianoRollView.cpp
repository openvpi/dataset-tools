        ...
        namespace ui {
            ...
            void PianoRollView::setupInputCallbacks() {
                ...
                cb.viewportZoomAt = [this](double center, double factor) {
                    ...
                    if (drawW <= 0)
                        return;
                    double newHScale = m_hScale * factor;

                    double minScale = 20.0;
                    if (m_audioDuration > 0) {
                        minScale = qMax(minScale, drawW / m_audioDuration);
                    }
                    newHScale = qBound(minScale, newHScale, 5000.0);

                    double viewDuration = drawW / newHScale;
                    double startSec = center - viewDuration * 0.5;
                    double endSec = startSec + viewDuration;
                    if (startSec < 0.0) {
                        startSec = 0.0;
                        endSec = viewDuration;
                    }
                    if (m_audioDuration > 0 && endSec > m_audioDuration) {
                        endSec = m_audioDuration;
                        startSec = qMax(0.0, endSec - viewDuration);
                    }

                    m_hScale = newHScale;
                    m_scrollX = startSec * m_hScale;
                    syncToViewport(startSec, endSec);
                    updateScrollBars();
                    update();
                };
                cb.viewportScrollBy = [this](double d) {
                    ...
                    if (drawW <= 0)
                        return;
                    double viewDuration = drawW / m_hScale;
                    double newStart = m_scrollX / m_hScale + d;
                    if (newStart < 0.0)
                        newStart = 0.0;
                    if (m_audioDuration > 0 && newStart + viewDuration > m_audioDuration)
                        newStart = m_audioDuration - viewDuration;
                    if (newStart < 0.0)
                        newStart = 0.0;
                    m_scrollX = newStart * m_hScale;
                    syncToViewport(newStart, newStart + viewDuration);
                    updateScrollBars();
                    update();
                };
                cb.viewportViewCenter = [this]() {
                    double drawW = width() - ScrollBarSize - PianoWidth;
                    return (m_scrollX + drawW * 0.5) / m_hScale;
                };
                cb.viewportSetPPS = [this](double pps) {
                    double drawW = width() - ScrollBarSize - PianoWidth;
                    if (drawW <= 0)
                        return;
                    double minScale = 20.0;
                    if (m_audioDuration > 0)
                        minScale = qMax(minScale, drawW / m_audioDuration);
                    double newHScale = qBound(minScale, pps, 5000.0);
                    double viewDuration = drawW / newHScale;
                    m_hScale = newHScale;
                    syncToViewport(m_scrollX / m_hScale, m_scrollX / m_hScale + viewDuration);
                    updateScrollBars();
                    update();
                };
            ...
            void PianoRollView::updateNoteMenuState() {
                ...
                m_actToggleRest->setText(note.isRest() ? QStringLiteral("转为普通音符(&R)")
                                                       : QStringLiteral("标记为休止(&R)"));

                m_actToggleSlur->setText(note.isSlur() ? QStringLiteral("取消连音(&L)")
            ...
            void PianoRollView::setAudioDuration(double sec) {
                ...
                if (m_audioDuration > 0) {
                    double drawW = width() - ScrollBarSize - PianoWidth;
                    if (drawW > 0) {
                        double fitScale = drawW / m_audioDuration;
                        if (m_hScale < fitScale) {
                            m_hScale = fitScale;
                        }
                        m_scrollX = 0.0;
                        syncToViewport(0.0, m_audioDuration);
                    }
                }
                updateScrollBars();
                update();
            ...
            void PianoRollView::zoomIn() {
                double drawW = width() - ScrollBarSize - PianoWidth;
                double centerSec = (m_scrollX + drawW * 0.5) / m_hScale;
                double factor = 1.2;
                double newHScale = m_hScale * factor;
                newHScale = qMin(newHScale, 5000.0);

                double viewDuration = drawW / newHScale;
                double startSec = centerSec - viewDuration * 0.5;
                double endSec = startSec + viewDuration;
                if (startSec < 0.0) {
                    startSec = 0.0;
                    endSec = viewDuration;
            ...
            void PianoRollView::zoomOut() {
                ...
                newHScale = qMax(newHScale, minScale);
            ...
            void PianoRollView::resetZoom() {
                ...
                if (m_audioDuration > 0 && drawW > 0) {
                    double minScale = drawW / m_audioDuration;
                    newScale = qMax(newScale, minScale);
                }
                newScale = qMin(newScale, 5000.0);
                m_hScale = newScale;
                m_vScale = 20.0;
                m_scrollX = 0.0;
                if (m_audioDuration > 0) {
            ...
            void PianoRollView::syncToViewport(double startSec, double endSec) {
                m_syncingViewport = true;
                m_viewport->setViewRange(startSec, endSec);
                m_syncingViewport = false;
            }
            ...
            void PianoRollView::setViewportController(dstools::widgets::ViewportController *vc) {
                ...
                m_viewport = vc;
                if (vc) {
                    connect(vc, &dstools::widgets::ViewportController::viewportChanged, this,
            ...
            void PianoRollView::onViewportChanged(const dstools::widgets::ViewportState &state) {
                ...
                double viewDuration = state.endSec - state.startSec;
                if (viewDuration > 0.0) {
                    m_hScale = drawW / viewDuration;
                }
