...
void PianoRollRenderer::drawGrid(QPainter &p, int w, int h, const RenderState &s) {
    ...
    for (int midi = s.minMidi; midi <= s.maxMidi; ++midi) {
        ...
        p.drawLine(s.contentLeft, wy, w, wy);
    }

    // 使用与标尺相同的逻辑来绘制网格线，避免过密
    const auto &level = findRulerLevel(s.pixelsPerSecond());
    ...
    // 只绘制主刻度线作为网格线
    double t = std::floor(tLeft / level.majorSec) * level.majorSec;
    while (t <= tRight + level.majorSec) {
        if (t >= 0) {
...
void PianoRollRenderer::drawNotes(QPainter &p, int w, int h, const RenderState &s) {
    ...
    for (int i = 0; i < static_cast<int>(s.dsFile->notes.size()); ++i) {
        ...
        if (wx2 < s.contentLeft || wx1 > w) continue;
        ...
        int drawX = qMax(s.contentLeft, wx1);
        ...
        if (s.showPhonemeTexts) {
            for (const auto &phone : s.dsFile->phones) {
                ...
                int phDrawX = qMax(s.contentLeft, phWx1);

                int phY = wy + noteH;
                int phH = fm.lineSpacing() + 3;

                QPen sepPen(dsfw::Theme::instance().palette().pianoRoll.phonemeSeparator);
                sepPen.setWidth(2);
                p.setPen(sepPen);
                p.drawLine(phDrawX + 1, phY + 1, phDrawX + 1, phY + phH);

                p.setPen(Qt::NoPen);
                p.setBrush(dsfw::Theme::instance().palette().pianoRoll.phonemeSeparatorFill);
    ...
    p.setPen(dsfw::Theme::instance().palette().pianoRoll.phonemeText);
...
void PianoRollRenderer::drawF0Curve(QPainter &p, int w, int h, const RenderState &s) {
    ...
    if (!path.isEmpty()) {
        if (s.abComparisonActive && s.originalF0 && !s.originalF0->empty()) {
            ...
            if (!origPath.isEmpty()) {
                QPen origPen(dsfw::Theme::instance().palette().pianoRoll.f0Original, 2);
...
void PianoRollRenderer::drawPlayhead(QPainter &p, int w, int h, const RenderState &s) {
    // 考虑DsPitchDocument的时间偏移，与F0曲线保持一致
    double playheadTime = s.playheadPos;
    if (s.dsFile) {
        double offsetSec = usToSec(s.dsFile->offset);
        playheadTime += offsetSec;
    }
...
void PianoRollRenderer::drawCrosshair(QPainter &p, int w, int h, const RenderState &s) {
    ...
    if (mx < s.contentLeft || mx > w || my < RenderState::RulerHeight || my > h) return;

    p.save();
    p.setClipRect(s.contentLeft, RenderState::RulerHeight,
                  w - s.contentLeft, h - RenderState::RulerHeight);

    QPen crossPen(dsfw::Theme::instance().palette().pianoRoll.crosshair, 1, Qt::DashLine);
    p.setPen(crossPen);
    p.drawLine(mx, RenderState::RulerHeight, mx, h);
    p.drawLine(s.contentLeft, my, w, my);
    ...
    p.setPen(Qt::NoPen);
    p.setBrush(dsfw::Theme::instance().palette().pianoRoll.tooltipBg);
    p.drawRoundedRect(textX, textY, textW, textH, 3, 3);

    p.setPen(dsfw::Theme::instance().palette().pianoRoll.tooltipText);
