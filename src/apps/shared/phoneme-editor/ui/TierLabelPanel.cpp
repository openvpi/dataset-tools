...
double TierLabelPanel::timeToX(double timeSec) const {
    return buildCoordinate().timeToX(timeSec);
}

ChartCoordinate TierLabelPanel::buildCoordinate() const {
    ChartCoordinate coord;
    coord.viewStart = m_viewStart;
    coord.viewEnd = m_viewEnd;
    coord.pixelWidth = m_pixelWidth;
    return coord;
}
...
void TierLabelPanel::drawRow(QPainter &painter, const TierLabelRow &row) {
    ...
    for (int i = 0; i < count - 1; ++i) {
        ...
        QString text = m_boundaryModel->intervalText(row.tierIndex, i);
        if (text.isEmpty())
            text = QStringLiteral("--");
...
void TierLabelPanel::mouseMoveEvent(QMouseEvent *event) {
    ...
    int bestIdx = -1;
    double bestDist = 1e9;
    double clickTime = m_viewStart + (m_viewEnd - m_viewStart) * event->pos().x() / std::max(1, m_pixelWidth);
...
void TierLabelPanel::wheelEvent(QWheelEvent *event) {
    if (event->modifiers() & Qt::ControlModifier && m_viewport) {
        double t = m_viewStart + (m_viewEnd - m_viewStart) * event->position().x() / std::max(1, m_pixelWidth);
