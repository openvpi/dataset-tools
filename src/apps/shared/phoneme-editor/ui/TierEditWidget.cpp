...
namespace dstools {
    namespace phonemelabeler {
        ...
        TierEditWidget::TierEditWidget(TextGridDocument *doc, QUndoStack *undoStack, ViewportController *viewport,
            ...
            // 添加到主布局（仅层级视图容器）
            mainLayout->addWidget(m_tierViewsContainer, 1);
        ...
        void TierEditWidget::updateRadioButtons() {
            ...
            while (radioLayout->count() > 0) {
                QLayoutItem *item = radioLayout->takeAt(0);
                if (item && item->widget()) {
        ...
        void TierEditWidget::setYAxisWidth(int w) {
            if (m_radioButtonContainer)
                m_radioButtonContainer->setFixedWidth(w);
        }
