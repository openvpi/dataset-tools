...
namespace dstools {
        ...
        public:
            ...
            explicit SpectrogramColorPalette(const QString &name, const std::vector<KeyColor> &keys) : m_name(name) {
                buildLUT(keys);
            }
            ...
            static std::vector<SpectrogramColorPalette> builtinPresets() {
                return {
                    orangeYellow(), grayscale(), sunset(), midnight(), dawn(), inferno(),
                };
            }
            ...
            void buildLUT(const std::vector<KeyColor> &keys) {
                ...
                for (size_t i = 1; i < keys.size(); ++i) {
                    ...
                    for (int s = 1; s <= stepCount; ++s) {
                        ...
                        expanded.push_back(qRgba(std::clamp(r, 0, 255), std::clamp(g, 0, 255), std::clamp(b, 0, 255),
                                                 std::clamp(a, 0, 255)));
                    }
                }
