...
| 方向 | 路线图 | 状态 |
|------|--------|------|
| 数据路径迁移 QStandardPaths（三平台路径统一） | H.5 | ✅ 已完成 |
| 统一 CrashHandler（替换 QBreakpad、修复 bug） | H.4 | ✅ 已完成 |
...
| 方向 | 路线图 | 说明 |
|------|--------|------|
| TODO/FIXME 清理 | B.2 | ✅ 已完成 |
| 文件操作错误处理 | B.3 | ⏳ 大部分已补全，少数 UI 便捷路径仍缺 else 分支 |
| 大文件拆分 | C.1 | PitchLabelerPage (755行), PhonemeLabelerPage (600行) |
| 魔法数字常量化 | C.2 | `4096` 3 处重复 |
| Doxygen CI | D.2 | ✅ 已完成（docs.yml 已配置） |
| 跨平台包分发 | D.3 | ✅ 已完成（release.yml 已配置） |
| 示例项目 | F.1 | 有外部用户需求时做 |
