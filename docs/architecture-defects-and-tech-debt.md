...
| 编号 | 描述 | 严重性 | 状态 |
|------|------|--------|------|
| AD-05 | dsfw-core 依赖 Qt::Gui (QColor 等) | Low | ✅ 已修复 |
| AD-06 | pipeline-pages 使用 `../slicer/` 相对路径源文件引用 | Low | ✅ 已修复 |

### AD-05: core 依赖 Qt::Gui

已修复 — dsfw-core CMakeLists 现在仅链接 Qt::Core 和 Qt::Network，不再依赖 Qt::Gui。

### AD-06: pipeline-pages 相对路径源文件引用

已修复 — slicer、lyricfa、hubertfa 已独立为静态库，pipeline pages 通过 target_link_libraries 链接。
...
| ARCH-08 | 应用层直接 #include 推理库头文件，UI 中管理推理生命周期 | High | ✅ 已修复 |
...
待验证 — DocumentFormat 已改为 DocumentFormatId 注册模式，需确认静态变量是否已移除。
...
| 编号 | 描述 | 严重性 | 状态 |
|------|------|--------|------|
| TD-02 | 领域模块部分测试覆盖 | 中 | 框架核心已有测试，部分领域类（CsvToDsConverter、TextGridToCsv、PitchProcessor 等）仍缺 |
...
**待测模块**: TranscriptionPipeline, CsvToDsConverter, TextGridToCsv, PitchProcessor

**已有测试**: TestResult, TestJsonHelper, TestAppSettings, TestServiceLocator, TestAsyncTask, TestLocalFileIOProvider, TestModelManager, TestModelDownloader, TestAppShellIntegration, TestDsDocument, TestF0Curve, TestPhNumCalculator, TestPitchUtils, TestTranscriptionCsv
