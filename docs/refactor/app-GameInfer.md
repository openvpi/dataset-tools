...
> **勘误**: 原 01-architecture.md 仅提到删除 `DmlGpuUtils.*`，遗漏了 `GpuInfo.h`。该文件定义了 `GpuInfo` 结构体（`index`, `name`, `dedicatedMemory`），已在共享 `GpuSelector.h` 中以 `dstools::widgets::GpuInfo` 重新定义。
...
> **勘误**: 原方案中 05-unified-app.md 提到 GameInfer.ini 但未说明这是新建的。原 GameInfer 代码中没有任何 QSettings 使用。此配置文件是重构新增的便利功能。
