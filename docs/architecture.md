...
| SlurCutter.exe | 句子/MIDI 编辑器 | 钢琴卷帘 F0 可视化、音符编辑 |
...
| dstools-widgets | SHARED (DLL) | 可视化控件、主题系统，四个应用共享 |
...
| hubert-infer | SHARED | HuBERT 音素对齐推理，从 HubertFA 提取 |
...
| `hubert-infer` | SHARED (规划中) | audio-util, nlohmann_json, dstools-infer-common |
...
> `hubert-infer` 尚未提取（AD-01），当前 Pipeline 的 HubertFA Tab 直接引用 `src/apps/HubertFA/util/` 中的源文件。
...
#### 5.2.1 MinLabel — 音频标注工具

入口：`src/apps/MinLabel/main.cpp`

面向歌声合成标注的轻量编辑器。

- **G2P 转换**：支持普通话（cpp-pinyin）、粤语、日语（cpp-kana）
- **JSON 标注编辑**：编辑歌声合成训练所需的标注文件
- **Lab 文件生成**：导出 HTK Label 格式
...
#### 5.2.2 SlurCutter — 句子/MIDI 编辑器

入口：`src/apps/SlurCutter/main.cpp`

DiffSinger 专用的音符与句子编辑工具。

- 钢琴卷帘式 F0 可视化
- 音符编辑（起止时间、音高）
- 连音（slur）与滑音（glide）标记
...
| cpp-pinyin | 普通话 G2P |
| cpp-kana | 日语 G2P |
| textgrid.hpp | Praat TextGrid 读写 |
...
| SomeModel | SlurCutter | SOME MIDI 估计 |
...
通过 `Theme::apply()` 切换深色/浅色 QSS 主题，`Theme::palette()` 提供语义化调色板。四个应用共享 dstools-widgets.dll 中的主题实现。
...
其余依赖（Qt、SDL2、SndFile、soxr、fftw3、yaml-cpp、nlohmann/json 等）通过 vcpkg 管理，见 `scripts/vcpkg-manifest/`。
