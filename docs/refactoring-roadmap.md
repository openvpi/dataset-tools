...
## Phase A — 可靠性与接口一致性 (P2)

### A.1 ServiceLocator 改用 std::any — P2, S (<1h)

**现状**: `void*` 存储 + `static_cast` 取出，类型不匹配时是未定义行为（静默返回错误指针），调试困难。

**方案**: 内部存储改为 `std::any`，取出时用 `std::any_cast<T*>`，类型不匹配返回 `nullptr`。约 10 行改动，API 不变。

```cpp
// 改前 (UB on type mismatch)
static_cast<T *>(it->second)
// 改后 (safe, returns nullptr on mismatch)
...
### A.2 服务接口一致性修复 — P2, S (2-4h)

五个后端服务接口模式不统一：

| 接口 | loadModel | isModelLoaded | unloadModel | 问题 |
|------|-----------|---------------|-------------|------|
| IAlignmentService | ✅ 虚方法 | ✅ | ✅ | — |
| ITranscriptionService | ✅ 虚方法 | ✅ | ✅ | — |
| IPitchService | ✅ 虚方法 | ✅ | ✅ | — |
| **IAsrService** | ❌ 缺失 | ❌ | ❌ | AsrService 自行添加了非虚方法，调用方无法通过接口管理模型 |
| ISlicerService | — | — | — | 无需模型，合理 |

**方案**:
1. IAsrService 补齐 `loadModel()`/`isModelLoaded()`/`unloadModel()` 虚方法
2. AsrService 改为 override

**同时修复**: `IInferenceEngine::load()` 有默认实现 `return Err("not implemented")`，改为纯虚 `= 0`。默认实现会掩盖子类忘记实现的编译错误。`unload()`/`terminate()` 保留默认空实现（合理）。

---

### A.3 CrashHandler 全量启用 — P2, S (<1h)

**现状**: CrashHandler 已实现（Windows MiniDump / Unix signal），但只有 MinLabel 一个 app 启用 (`AppInit::init(app, /*initCrashHandler=*/true)`)。其余 5 个 app 传 false 或使用默认值。

**方案**: 所有 app 的 `AppInit::init()` 调用改为 `initCrashHandler=true`。
...
1. `CMakeLists.txt`: `set(CMAKE_CXX_STANDARD 20)`
2. README.md 更新编译器要求
3. CI 验证三平台通过
...
```
P2 — 有实际价值
  A.1  ServiceLocator 类型安全 (S) — 消除 void* UB
  A.2  服务接口一致性修复 (S)     — IAsrService 补齐虚方法
  A.3  CrashHandler 全量启用 (S)  — 所有 app 启用崩溃收集
  B.1  补齐领域测试 (L)           — 防止回归
...
```
批次 1: A.1 + A.2 + A.3 (接口/稳定性) + E.1 (C++20) — 小改动，立竿见影
...
| 编号 | 描述 | 严重性 |
|------|------|--------|
| TD-01 | ServiceLocator 使用 void* 存储，类型不匹配时 UB | 中 |
| TD-02 | IAsrService 缺模型生命周期虚方法，与其他服务接口不一致 | 中 |
| TD-03 | 4 个领域模块缺测试 | 中 |
| TD-04 | 部分文件操作缺错误分支 | 低 |
| TD-05 | 2 个文件超 600 行 | 低 |
| TD-06 | `4096` buffer size 3 处重复 | 低 |
| TD-07 | 5 处 TODO/FIXME | 低 |
| TD-08 | IInferenceEngine::load() 非纯虚，子类可意外不实现 | 低 |
| TD-09 | CrashHandler 仅 1/6 app 启用 | 低 |
...
| # | 任务 | 工作量 | 涉及文件 | 并行 | 关联 |
|---|------|--------|---------|------|------|
| 1 | **A.1** ServiceLocator void* → std::any | S (<1h) | `ServiceLocator.h` (~10 行) | ✅ 可并行 | TD-01 |
| 2 | **A.2** IAsrService 补齐 loadModel/isModelLoaded/unloadModel 虚方法；IInferenceEngine::load() 改纯虚 | S (2-4h) | `IAsrService.h`, `AsrService.h/.cpp`, `IInferenceEngine.h` | ✅ 可并行 | TD-02, TD-08 |
| 3 | **A.3** CrashHandler 全量启用 | S (<1h) | 5 个 app 的 `main.cpp` (各改 1 行) | ✅ 可并行 | TD-09 |
| 4 | **E.1** CMake 升级 C++20 + README 更新编译器要求 | S (<2h) | `CMakeLists.txt`, `README.md` | ✅ 可并行 | — |

> 批次 1 全部互不依赖，4 个任务可同时进行。
