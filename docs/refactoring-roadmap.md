...
| M.3.6 | DsPhonemeLabelerPage | ProjectDataSource + PhonemeEditor + 自动 FA 逻辑 + 批处理菜单 + 预加载 | 中：预加载需后台线程池 + UI 取消同步 | 依赖 M.1.2, M.3.1 | ✅ (saveCurrentSlice → dstext 已实现) |
| M.3.7 | DsPitchLabelerPage | ProjectDataSource + PitchEditor + 自动 add_ph_num/F0/MIDI + 批处理 + 预加载 | 中：3 个自动步骤串联，错误处理复杂 | 依赖 M.1.3, M.3.1 | ✅ |
| M.3.8 | ExportPage | 导出页面 UI + 自动补全逻辑 + CSV/DS/WAV 输出 + 重采样 + 进度 | 中：补全逻辑需调用多个处理器，重采样需 soxr 跨平台验证 | 依赖 M.3.1, L.9 | ✅ (CSV/DS/WAV 导出已实现) |
...
| M.5.6 | DsSlicerPage 骨架 | 新建 `src/apps/ds-labeler/DsSlicerPage.{h,cpp}`。布局：预处理折叠面板 + 切片参数面板 + WaveformPanel + SliceNumberLayer + SliceListPanel | 中 | 依赖 M.5.1 | ✅ (onActivated 音频加载已实现) |
...
| M.5.10 | 切片导出对话框 | 弹出对话框选择：位深（16/24/32-bit PCM, 32-bit float）、声道（单声道/保持原始）、输出目录、命名前缀、序号位数。调用 SlicerService 写出切片 WAV | 低 | 依赖 M.5.8 | ✅ (libsndfile WAV 写入已实现) |
