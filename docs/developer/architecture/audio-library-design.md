...
> 版本：1.1.0
> 日期：2026-06-04（方案设计），2026-06-08（实施完成更新）
> 状态：已实施
> 依赖：FFmpeg（解码 + 重采样），无 Qt 依赖（仅核心库，播放适配层可选）
>
> **实施摘要**：核心库 `dsfw-audio`（位于 `src/framework/audio/`）已完成实施，包括：
> - `AudioBuffer` / `AudioFormatInfo` / `ResampleConfig` — 类型定义层
> - `FfmpegAudioDecoder` (PIMPL) — FFmpeg 流式解码器
> - `SwresampleResampler` (PIMPL) — swresample 重采样器
> - `AudioPipeline` — 组合层（解码 + 重采样 + 文件写入）
> - `AudioFileWriter` — 多格式文件写入
> - `AudioPlaybackAdapter` / `IAudioPlayerAdapter` / `AudioPlayerAdapter` — 播放适配层
> - 单元测试覆盖：`TestAudioBuffer` / `TestAudioDecoder` / `TestAudioPipeline` / `TestAudioResampler`
>
> **与方案差异**：方案中计划的 `IAudioDecoder` / `IAudioResampler` 抽象接口未实施（当前仅有具体类，遵循 INFRA-03「先具体类再抽取接口」原则）。
...
**R16 已实施完成**（2026-06-08），核心库 `dsfw-audio` 位于 `src/framework/audio/`。
...
| 子阶段 | 名称 | 工作量 | 风险 | 依赖 | 状态 |
|--------|------|--------|------|------|------|
| R16.1 | AudioBuffer + 类型定义 | 2d | 低 | 无 | ✅ 已完成 |
| R16.2 | IAudioDecoder / IAudioResampler 接口 | 1d | 低 | R16.1 | ⏭️ 跳过（遵循 INFRA-03 先具体类再抽取接口） |
| R16.3 | FfmpegAudioDecoder (PIMPL) | 3d | 中 | R16.2 | ✅ 已完成 |
| R16.4 | SwresampleResampler (PIMPL) | 2d | 中 | R16.2 | ✅ 已完成 |
| R16.5 | AudioPipeline 组合层 | 1d | 低 | R16.3, R16.4 | ✅ 已完成 |
| R16.6 | 单元测试 | 2d | 低 | R16.5 | ✅ 已完成 |
| R16.7 | 应用层迁移（WaveformRenderer） | 1d | 低 | R16.5 | ✅ 已完成 |
| R16.8 | 应用层迁移（SlicerPage/SlicerService） | 1d | 中 | R16.5 | ✅ 已完成 |
| R16.9 | 推理层迁移（RMVPE, audio-util 替换） | 1d | 中 | R16.5 | ✅ 已完成 |
| R16.10 | 旧代码清理 | 1d | 中 | R16.7~R16.9 | ✅ 已完成 |
