...
- **保留全部 11 个页面**（含 Align、MIDI、CSV、DS 等 DsLabeler 中被合并/自动化的步骤的独立界面，以及 Settings 和 Log 页）
...
侧边栏 11 个图标（含 Settings 和 Log）。Settings 页与 DsLabeler 共享同一组件，但配置持久化到 `dsfw::AppSettings`（而非
.dsproj）。
...
| Settings 页（模型配置）                      | ✅ 有（持久化到 AppSettings）        | ✅ 有（持久化到 AppSettings）        |
...
侧边栏 8 个图标。打开工程前仅显示欢迎页，其余页面灰显不可用。
...
**切片页面本质上是单层的、不带音素名而是带数字序号的 PhonemeLabeler**。底层复用共享的 `AudioVisualizerContainer` 组件（见
§11），与 PhonemeLabeler 共享波形/频谱渲染和播放交互。额外功能仅为：导出切片音频（支持 WAV 格式选项）。
...
- **有输入**（导入 au 切点文件 / 已有 slices 层）→ 按导入的切点位置切分音频。波形图显示导入的切割线，用户可手动微调。"
  自动切片"按钮变为"仅对选中片段自动切片"（用于处理单个过长片段）。
...
> **全流程建议单声道**：DiffSinger 训练数据为单声道。切片导出默认单声道；后续所有步骤均以单声道数据工作。双声道音频的处理见
> §11.3。
...
横向 TabWidget，每个 tab 对应需要独立配置的项目。所有配置持久化到 `dsfw::AppSettings`（QSettings 后端），不再使用 `.dsproj` 的
`defaults` 字段。

| Tab         | 配置项                                           | AppSettings key                                                                                |
|-------------|-----------------------------------------------|------------------------------------------------------------------------------------------------|
| **设备**      | 全局推理提供者 (CPU/DML/CUDA暂禁用)、具体设备（名称+显存，过滤<1GB）  | `Models/globalProvider` + `Models/deviceIndex`                                                 |
| **ASR**     | ASR 模型路径、[Test] 按钮、☑ CPU 强制                   | `Models/asr_path`                                                                              |
| **词典/G2P**  | 各语言词典路径、音素表路径                                 | `languages[]`                                                                                  |
| **强制对齐**    | FA 模型路径、[Test] 按钮、☑ CPU 强制、☑ 预加载 [N] 个文件      | `Models/phoneme_alignment_path` + `Preload/phoneme_alignment`                                  |
| **音高/MIDI** | F0/MIDI 模型路径、[Test] 按钮、☑ CPU 强制、☑ 预加载 [N] 个文件 | `Models/pitch_extraction_path` + `Models/midi_transcription_path` + `Preload/pitch_extraction` |
| **预处理**     | 预处理器列表（响度归一化、降噪）、参数                           | `Preprocessors/...`                                                                            |
...
音素标注页使用共享的 `AudioVisualizerContainer` 组件（见 §11），交互行为统一：
...
> 实际代码参见 `src/apps/ds-labeler/main.cpp` 和 `src/apps/ds-labeler/CMakeLists.txt`。
...
**核心决策（ADR-66）**：LabelSuite 和 DsLabeler 底层使用**完全相同的数据模型** —
dstext/PipelineContext。两者共享同一套页面组件代码。区别仅在数据源（`FileDataSource` vs `ProjectDataSource`
）和应用壳层（页面选择、菜单、配置持久化）。
...
PhonemeEditor 内部使用 `TextGridDocument` 作为统一编辑模型。页面层负责 dstext ↔ TextGridDocument 转换（已由
`TextGridDocument::loadFromDsText()` / `toDsText()` 实现）。
...
| AudioVisualizerContainer | `src/apps/shared/audio-visualizer/`  | 统一音频可视化容器（MiniMap + TimeRuler + TierLabel + 子图 QSplitter + BoundaryOverlay） |
...
Settings 页面 UI 完全共享，统一使用 `AppSettingsBackend`（QSettings 后端）：

|             | LabelSuite                     | DsLabeler                      |
|-------------|--------------------------------|--------------------------------|
| Settings UI | ✅ 共享 SettingsWidget            | ✅ 共享 SettingsWidget            |
| 持久化         | `dsfw::AppSettings`（QSettings） | `dsfw::AppSettings`（QSettings） |

```cpp
// 两个应用统一使用 AppSettingsBackend（QSettings 后端）
// ISettingsBackend 抽象接口已在 Phase R.1 中删除（仅单一实现，无需抽象）
class AppSettingsBackend { /* QSettings */ };
```

> **注**：`ProjectSettingsBackend`（.dsproj 后端）已在 Phase R.1 中废弃删除。`ISettingsBackend`
> 抽象接口已随单一实现原则（INFRA-02/03）一并删除。两个应用统一使用 `AppSettingsBackend` 具体类。

---

> 原独立应用（MinLabel, PhonemeLabeler, PitchLabeler, DatasetPipeline, GameInfer, DiffSingerLabeler）已全部整合到
> LabelSuite 或 DsLabeler 中。保留 `dstools-cli.exe` 和 `WidgetGallery.exe`。
...
原 DiffSingerLabeler 的"制作声学数据集"步骤被省略。各步骤产出以层数据形式暂存在 PipelineContext（`dstemp/contexts/*.json`
）。导出 CSV 时按需自动执行 add_ph_num，通过 CsvAdapter 生成。
...
用户可在任意页面之间**自由跳转**。回退到上游页面修改数据后，下游自动标记 dirty，下次进入下游页面时自动重算并 Toast
提示。详见 [ds-format.md §6](ds-format.md)。
...
> 各页面菜单项由对应 Page 类的 `createMenuBar()` 提供，AppShell 在页面切换时自动更换菜单栏。
...
> **架构变更（D-01）**：Phase R.1 已将配置统一到 `dsfw::AppSettings`（QSettings 后端）。`ProjectSettingsBackend` 已废弃删除，
`.dsproj` 的 `defaults` 字段不再写入（读取时自动迁移）。详见 [ds-format.md §3.5](ds-format.md)。

两个应用均使用 `dsfw::AppSettings` 持久化配置，后端为 `AppSettingsBackend`（QSettings）：

```json
// AppSettings 存储结构（QSettings ini 格式）
[Models]
asr_path=...
phoneme_alignment_path=...
pitch_extraction_path=...
midi_transcription_path=...
global_provider=dml
device_index=0
...
`.dsproj` 仅保留工程数据（items、slicer、export 等），不再存储配置：
...
LabelSuite 和 DsLabeler 共享同一套 `AppSettings`，配置跨工程保持一致。
...
> **架构变更**：Phase S 已全部完成。`AudioVisualizerContainer` 统一可视化基类、`TierLabelArea` 标签区域层次结构（含
`SliceTierLabel` 和 `PhonemeTextGridTierLabel`）、`MiniMapScrollBar`、`BoundaryOverlayWidget`
> 贯穿规则、刻度线重设计、键鼠交互统一等均已实现。Slicer 和 PhonemeLabeler 均已迁移到该架构。`TierLabelArea` 和
`PhonemeTextGridTierLabel` 源文件位于 `src/apps/shared/phoneme-editor/ui/`，`SliceTierLabel` 位于
`src/apps/shared/audio-visualizer/`。

### 11.1 设计动机

切片页面和音素标注页面共享大量 UI 组件：波形图、时间刻度尺、频谱图、边界线、播放逻辑。通过 `AudioVisualizerContainer`
统一基类 + `IBoundaryModel` 抽象接口解耦，可视化组件可在切片和音素标注两个场景中直接复用。

### 11.2 AudioVisualizerContainer 统一布局
...
`AudioVisualizerContainer` 统一管理 MiniMapScrollBar（缩略图）、TimeRulerWidget（刻度尺）、TierLabelArea（标签区域）、QSplitter（子图列表）和
BoundaryOverlayWidget（边界线叠加层）。所有子图通过 `addChart()` 注册，可见性和排列顺序由用户配置。

标签区域分两种：`SliceTierLabel`（Slicer 用，单层自动编号）和 `PhonemeTextGridTierLabel`（PhonemeLabeler 用，多层 + radio
button 选中层贯穿）。详见 `component-design.md`。

### 11.4 Slicer/PhonemeLabeler 具体配置

| 组件                |     Slicer     |      PhonemeLabeler      | PitchLabeler |
|-------------------|:--------------:|:------------------------:|:------------:|
| MiniMapScrollBar  |       ✅        |            ✅             |    ❌ 独立架构    |
...
- **右键播放**：任意区域右键点击 → 播放所在边界区间。停止通过再次右键或 Escape
- **双声道处理**：默认混合为单声道，顶部提示条可切换分离显示；后续处理始终使用单声道数据
- **时间刻度尺**：TimeRulerWidget 自适应缩放层级（1h/10min/1min/10s/1s/100ms/10ms）
...
| 控件                     | 位置             | 用途                                   |
|------------------------|----------------|--------------------------------------|
| DroppableFileListPanel | `dsfw-widgets` | 可拖拽文件列表，支持过滤 + 进度条                   |
| PathSelector           | `dsfw-widgets` | 统一文件/目录选择控件，带路径记忆                    |
| PathUtils              | `dsfw-core`    | 跨平台路径编码（Windows wchar/macOS NFD→NFC） |
| SliceListPanel         | `dsfw-widgets` | 切片列表 + 标注进度条                         |
...
- [data-flow-design.md](../data-flow/data-flow-design.md) — 完整数据流设计（合并后）
- [ds-format.md](../data-flow/ds-format.md) — 工程格式规范

> 更新时间：2026-05-20 — §9 更新配置持久化（D-01：统一到 AppSettings）；§11 更新可视化架构（Phase S：AudioVisualizerContainer
> 已实现）
