...
侧边栏 11 个图标（含 Settings 和 Log）。Settings 页与 DsLabeler 共享同一组件，但配置持久化到 `dsfw::AppSettings`（而非 .dsproj）。
...
**切片页面本质上是单层的、不带音素名而是带数字序号的 PhonemeLabeler**。底层复用共享的 `AudioVisualizerContainer` 组件（见 §11），与 PhonemeLabeler 共享波形/频谱渲染和播放交互。额外功能仅为：导出切片音频（支持 WAV 格式选项）。
...
横向 TabWidget，每个 tab 对应需要独立配置的项目。所有配置持久化到 `dsfw::AppSettings`（QSettings 后端），不再使用 `.dsproj` 的 `defaults` 字段。
...
int main(int argc, char *argv[]) {
    ...
    shell.addPage(new WelcomePage(&shell),                "welcome", {}, "工程");
    shell.addPage(new DsSlicerPage(&shell),               "slicer",  {}, "切片");
    auto *minLabelPage = new MinLabelPage(&shell);
    minLabelPage->setDataSource(dataSource, settingsBackend);
    shell.addPage(minLabelPage,                           "minlabel",{}, "歌词");
    auto *phonemePage = new PhonemeLabelerPage(&shell);
    phonemePage->setDataSource(dataSource, settingsBackend);
    shell.addPage(phonemePage,                            "phoneme", {}, "音素");
    auto *pitchPage = new PitchLabelerPage(&shell);
    pitchPage->setDataSource(dataSource, settingsBackend);
    shell.addPage(pitchPage,                              "pitch",   {}, "音高");
    shell.addPage(new ExportPage(&shell),                 "export",  {}, "导出");
    shell.addPage(new SettingsPage(settingsBackend, &shell),"settings",{},"设置");
    shell.addPage(new LogPage(&shell),                    "log",     {}, "日志");
...
> **注**：`ProjectSettingsBackend`（.dsproj 后端）已在 Phase R.1 中废弃删除。两个应用统一使用 `AppSettingsBackend`。
...
> **架构变更（D-01）**：Phase R.1 已将配置统一到 `dsfw::AppSettings`（QSettings 后端）。`ProjectSettingsBackend` 已废弃删除，`.dsproj` 的 `defaults` 字段不再写入（读取时自动迁移）。详见 [ds-format.md §3.5](ds-format.md)。
...
> **架构变更**：Phase S 已全部完成。`AudioVisualizerContainer` 统一可视化基类、`TierLabelArea` 标签区域层次结构（含 `SliceTierLabel` 和 `PhonemeTextGridTierLabel`）、`MiniMapScrollBar`、`BoundaryOverlayWidget` 贯穿规则、刻度线重设计、键鼠交互统一等均已实现。Slicer 和 PhonemeLabeler 均已迁移到该架构。`TierLabelArea` 和 `PhonemeTextGridTierLabel` 源文件位于 `src/apps/shared/phoneme-editor/ui/`，`SliceTierLabel` 位于 `src/apps/shared/audio-visualizer/`。

### 11.1 设计动机

切片页面和音素标注页面共享大量 UI 组件：波形图、时间刻度尺、频谱图、边界线、播放逻辑。通过 `AudioVisualizerContainer` 统一基类 + `IBoundaryModel` 抽象接口解耦，可视化组件可在切片和音素标注两个场景中直接复用。
...
**所有使用 AudioVisualizerContainer 的页面统一交互**：
...
波形图上方固定显示时间刻度尺（`TimeRulerWidget`，已在 AudioVisualizerContainer 中实现）。刻度随缩放级别自适应，使用基于时间的刻度算法（参考 ds-editor-lite `ITimelinePainter`）：
...
> 更新时间：2026-05-20 — §9 更新配置持久化（D-01：统一到 AppSettings）；§11 更新可视化架构（Phase S：AudioVisualizerContainer 已实现）
