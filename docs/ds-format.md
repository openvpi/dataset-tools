...
**v3 变更**：
- 移除 `tasks[]` — 流水线步骤在代码中固定定义，不在工程文件中声明。每个步骤可选用哪个处理器通过 `defaults.models` 配置。
- 新增 `defaults.preload`、`defaults.export.resampleRate`。
- 路径字段统一用 POSIX 正斜杠。
...
"defaults": {
    ...
    "preprocessors": [
        {"id": "loudness_norm", "params": {"targetLufs": -23.0}}
    ],
    "models": {
        "asr": {
            "processor": "funasr",
    ...
    "preload": {
        "phoneme_alignment": {"enabled": false, "count": 10},
        "pitch_extraction": {"enabled": false, "count": 10}
    },
    "export": {
        "formats": ["csv", "ds"],
        "hopSize": 512,
...
**`defaults.models` 设计说明**：

key 为任务名称（`asr`、`phoneme_alignment`、`midi_transcription`、`pitch_extraction`），value 指定选用哪个处理器。每个任务在 `TaskProcessorRegistry` 中可注册多个处理器（如 `phoneme_alignment` 可选 `hubert-fa`、`sofa`、`mfa`），I/O 契约相同，模型可替换。
...
3. 用户唯一的配置自由度是**选用哪个处理器**（如 hubert-fa vs sofa），这通过 `defaults.models` 配置

因此 `tasks[]` 是死配置，增加复杂度但不提供价值。移除后：
- 步骤定义在代码中（`PipelineStepRegistry`）
- .dsproj 仅配置每步选用的处理器（`defaults.models`）
