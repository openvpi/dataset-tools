...
切片页面和音素标注页面共享大量 UI 组件：波形图、时间刻度尺、频谱图、边界线、播放逻辑。通过 `IBoundaryModel` 抽象接口解耦，phoneme-editor 的可视化组件可在切片和音素标注两个场景中直接复用。

> **注**：早期设计使用独立的 `WaveformPanel` 组件，已被 phoneme-editor 内的组件 + IBoundaryModel 替代（ADR-65）。`src/apps/shared/waveform-panel/` 仍存在但不再被 DsSlicerPage 使用。

### 11.2 组件层次（实际实现）

```
phoneme-editor (src/apps/shared/phoneme-editor/)
├── PhonemeEditor              ← 组合容器（音素标注页直接使用）
│   ├── TimeRulerWidget        ← 时间刻度尺（共享 ViewportController 同步缩放）
│   ├── WaveformWidget         ← 波形渲染 + 边界编辑 + 右键播放 + Ctrl+滚轮振幅缩放
│   ├── PowerWidget            ← 功率曲线
│   ├── SpectrogramWidget      ← FFT 频谱图 + 右键播放
│   ├── TierEditWidget         ← 多层 TextGrid 编辑
│   ├── BoundaryOverlayWidget  ← 贯穿所有图的边界线 + 播放游标
│   └── EntryListPanel         ← 条目列表
├── IBoundaryModel             ← 抽象接口（边界数据源）
├── TextGridDocument           ← IBoundaryModel 实现（音素标注用）
└── SliceBoundaryModel         ← IBoundaryModel 实现（切片用）

DsSlicerPage（切片页，直接使用 phoneme-editor 的 widget）
├── TimeRulerWidget            ← 复用
├── WaveformWidget             ← 复用，setBoundaryModel(SliceBoundaryModel)
├── SpectrogramWidget          ← 复用
├── SliceNumberLayer           ← 切片序号标注
└── ViewportController         ← 共享缩放/滚动
```
...
> 更新时间：2026-05-03 — §11 更新为实际架构（IBoundaryModel + phoneme-editor 组件复用），废弃 WaveformPanel 描述
