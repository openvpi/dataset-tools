...
向导完成后创建 `.dsproj` + `audio/` 目录，自动跳转到**切片页面**。切片操作在独立的切片页面完成（而非向导内嵌），用户可随时返回调参重切。
...
│                                                                            │
│  ┌─ 切片参数（内嵌面板） ──────────────────────────────────────────────┐  │
│  │ Threshold: [-40] dB   Min Length: [5000] ms   Min Interval: [300] ms   │
│  │ Hop Size: [10] ms     Max Silence: [500] ms                            │
│  │                                                                         │
│  │ [导入切点...]  [自动切片]  [重新切片]  [保存切点...]  [导出音频...]     │
│  └─────────────────────────────────────────────────────────────────────────┘
│                                                                            │
│  ┌─ 时间刻度尺 ────────────────────────────────────────────────────────┐  │
...
│  ┌─ 切片列表（下方面板） ──────────────────────────────────────────────┐  │
│  │  # │ ID              │ 时长    │ 状态                                │  │
│  │  1 │ guangnian_001   │ 5.2s   │ ✓                                   │  │
│  │  2 │ guangnian_002   │ 35.1s  │ ⚠ 过长                              │  │
│  │  3 │ guangnian_003   │ 0.2s   │ 🗑 已丢弃                           │  │
│  └─────────────────────────────────────────────────────────────────────────┘
└──────────────────────────────────────────────────────────────────────────┘
```
...
> **已移除**：切片 tab。切片参数直接内嵌在切片页面（§3.4），不在 Settings 中重复配置。
...
```
[欢迎页] ─── 新建工程：选择音频 → 创建 .dsproj
  │
  ▼
[切片页] ─── 自动切片 + 手动切片 + 导出切片音频
  │         ├─ 参数内嵌（无需设置页）
  │         ├─ 可选输入层（导入切点文件）
  │         └─ 右键播放片段
  │
