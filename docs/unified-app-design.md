...
### 4.2 DsLabeler 核心编辑器复用

**设计原则**：DsLabeler 的标注页面与 LabelSuite **完全复用同一个编辑器 UI**（波形图、频谱图、时间刻度、边界拖拽、钢琴卷帘等），**仅文档后端不同**。DsLabeler 维护 dsitem/dstext 格式；LabelSuite 保持独立的 TextGrid / .ds 文件格式。

#### 架构：PhonemeEditor 的文档适配
...
**关键决策**：PhonemeEditor 内部使用 `TextGridDocument` 作为统一编辑模型。DsLabeler 页面负责在 dsitem/dstext ↔ TextGridDocument 之间做格式转换：

```
LabelSuite PhonemeLabelerPage：
  文件系统 .TextGrid ──→ TextGridDocument::load() ──→ PhonemeEditor
  PhonemeEditor ──→ TextGridDocument::save() ──→ .TextGrid 文件

DsLabeler DsPhonemeLabelerPage：
  ProjectDataSource ──→ dstext → TextGridDocument（内存转换）──→ PhonemeEditor
  PhonemeEditor ──→ TextGridDocument → dstext（内存转换）──→ ProjectDataSource::saveSlice()
```

这样 PhonemeEditor 无需感知数据来源差异。`DsPhonemeLabelerPage` 的职责：
1. 从 `ProjectDataSource` 加载 dstext 层数据
2. 将 dstext 的 `IntervalLayer` 转为 `TextGridDocument` 的 tier（调用 `TextGridDocument::fromDsText()`）
3. 传递给 `PhonemeEditor` 编辑
4. 保存时从 `TextGridDocument` 提取 tier 数据转回 dstext 格式，写入 `ProjectDataSource`

#### 页面对应关系

| LabelSuite 页面 | DsLabeler 页面 | 共享编辑器 | 文档后端差异 |
|---|---|---|---|
| PhonemeLabelerPage | DsPhonemeLabelerPage | PhonemeEditor | TextGrid 文件 vs dstext 层 |
| PitchLabelerPage | DsPitchLabelerPage | PitchEditor | .ds 文件 vs dstext curves |
| — | DsSlicerPage | WaveformPanel | 独立切点管理 vs dsitem |
...
public:
    /// 从 dstext 层数据构建内存 TextGrid。
    /// @param layers  dstext 的 IntervalLayer 列表
    /// @param duration 音频总时长（微秒），用于设置 TextGrid 的 maxTime
    void loadFromDsText(const QList<IntervalLayer> &layers, TimePos duration);

    /// 导出当前所有 tier 为 dstext IntervalLayer 列表。
...
| ProjectDataSource | `src/apps/ds-labeler/` | 工程 + PipelineContext 实现（DsLabeler 专用） |
| PhonemeEditor | `src/apps/shared/phoneme-editor/` | 核心 TextGrid 编辑逻辑，波形/频谱，不含文件 I/O |
...
> 更新时间：2026-05-03 — §4.2 明确 DsLabeler 复用架构：PhonemeEditor 为统一 UI，DsPhonemeLabelerPage 负责 dstext ↔ TextGridDocument 转换；移除 IEditorDataSource 抽象层（过度设计），改为直接在页面层做格式转换
