...
- **保留全部 10 个页面**（含 Align、MIDI、CSV、DS 等 DsLabeler 中被合并/自动化的步骤的独立界面，以及 Settings 页）
...
侧边栏 10 个图标（含 Settings）。Settings 页与 DsLabeler 共享同一组件，但配置持久化到 `dsfw::AppSettings`（而非 .dsproj）。
...
> **架构变更（D-01）**：Phase R.1 已将配置统一到 `dsfw::AppSettings`（QSettings 后端）。`ProjectSettingsBackend` 已废弃删除，`.dsproj` 的 `defaults` 字段不再写入（读取时自动迁移）。详见 [refactoring-v2.md §3](refactoring-v2.md)。
...
> **架构变更**：Phase S 已全部完成。`AudioVisualizerContainer` 统一可视化基类、`TierLabelArea` 标签区域层次结构（含 `SliceTierLabel` 和 `PhonemeTextGridTierLabel`）、`MiniMapScrollBar`、`BoundaryOverlayWidget` 贯穿规则、刻度线重设计、键鼠交互统一等均已实现。Slicer 和 PhonemeLabeler 均已迁移到该架构。`TierLabelArea` 和 `PhonemeTextGridTierLabel` 源文件位于 `src/apps/shared/phoneme-editor/ui/`，`SliceTierLabel` 位于 `src/apps/shared/audio-visualizer/`。详见 [refactoring-v2.md §2](refactoring-v2.md)。
...
- [refactoring-v2.md](refactoring-v2.md) — 重构设计方案（含 AudioVisualizerContainer 架构和配置迁移）
- [refactoring-roadmap-v2.md](refactoring-roadmap-v2.md) — 重构路线图与任务清单
- [migration-guide.md](migration-guide.md) — AppShell 迁移指南

> 更新时间：2026-05-05 — §9 更新配置持久化（D-01：统一到 AppSettings）；§11 更新可视化架构（Phase S：AudioVisualizerContainer 已实现）
