...
Hz↔MIDI、音名↔MIDI 转换在三处独立实现，存在微妙不一致，可能导致同一 .ds 文件在不同应用中音高显示偏差。

- **位置**:
  - `SlurCutter/F0Widget.cpp:360-410` — `FrequencyToMidiNote`、`NoteNameToMidiNote`、`MidiNoteToNoteName`（查表法）
  - `PitchLabeler/ui/PianoRollView.cpp:48-96` — `freqToMidi`、`midiToFreq`、`parseNoteName`（正则法，支持 cents）
  - `PitchLabeler/DSFile.cpp` — 加载/保存时内联 Hz↔MIDI 转换
- **风险**: SlurCutter 与 PitchLabeler 的 cents 精度处理不同，跨应用打开同一文件时可能显示不一致
- **目标**: 在 `dstools-core` 新建 `PitchUtils.h`，统一 `freqToMidi`/`midiToFreq`/`midiToNoteName`/`noteNameToMidi`（含 cents 支持），三处全部改为调用公共实现
- **工作量**: S（半天）
- **阻塞**: 无

### AD-09: 音频文件关联逻辑不一致

三个应用各自实现「根据数据文件找对应音频」的逻辑，且存在跨应用兼容性问题。

- **位置**:
  - `MinLabel/Common.cpp:17-44` — `labFileToAudioFile()`：扫描目录匹配同名 wav/mp3/m4a/flac
  - `SlurCutter/MainWindow.cpp:24-29` — `audioFileToDsFile()`：反向映射，将音频后缀编码进 .ds 文件名（`song_mp3.ds`）
  - `PitchLabeler/MainWindow.cpp:673-682` — 内联代码：简单替换 .ds 后缀尝试各种音频扩展名
- **风险**: SlurCutter 把音频后缀编码进 .ds 文件名（如 `song_mp3.ds`），但 PitchLabeler 不知道这个约定，用简单扩展名替换，**无法找到非 wav 音频文件**。这是一个潜在的跨应用兼容性 Bug
...
- **位置**:
  - `SlurCutter/MainWindow.cpp:56-73` — 快捷键注册
  - `MinLabel/MainWindow.cpp:56-75` — 快捷键注册
  - `PitchLabeler/MainWindow.cpp:144-182` — 快捷键注册
...
### AD-12: SlurCutter/MinLabel 文件浏览树重复

SlurCutter 和 MinLabel 的 `QTreeView + QFileSystemModel + 列隐藏 + FileStatusDelegate + 目录打开` 逻辑几乎逐行相同（~60 行）。

- **位置**:
  - `SlurCutter/MainWindow.cpp:111-132` — QTreeView + QFileSystemModel 设置
  - `MinLabel/MainWindow.cpp:113-146` — 几乎相同的 QTreeView + QFileSystemModel 设置
  - 两者均隐藏相同列、设置相同过滤器（`*.wav/*.mp3/*.m4a/*.flac`）、创建 `FileStatusDelegate`
- **目标**: 在 `dstools-widgets` 抽取为 `AudioFileBrowser` 组件，配置项为文件过滤器和状态检查回调
- **注意**: PitchLabeler 的 `FileListPanel` 是不同设计（QListWidget，浏览 .ds 文件），不应强制统一
- **工作量**: S（半天）
- **阻塞**: 无
...
| 项 | 原因 |
|---|---|
| DS 字段解析（note_seq 拆分等） | SlurCutter 保留原始字符串只改四个字段，PitchLabeler 需完整解析为 typed struct。抽象层次不同，强行统一会过度设计 |
| 主窗口基类 `BaseLabelerWindow` | 三个应用的菜单结构、状态栏、布局差异较大，抽基类会过度耦合 |
| 拖拽打开目录 | 仅 ~20 行实际逻辑（SlurCutter/MinLabel 的 dragEnterEvent/dropEvent），提取为 mixin 的成本约等于收益 |
...
AD-08 (PitchUtils)
  └── 解锁 → PitchLabeler/SlurCutter 音高显示一致性

AD-09 (AudioFileResolver)
  └── 修复 → PitchLabeler 无法打开 SlurCutter 产生的非 wav 文件
