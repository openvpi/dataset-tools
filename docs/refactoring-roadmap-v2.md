...
### V.6 IEditorDataSource 层统一音频文件有效性管理（D-26）
...
### V.7 dsproj audioSource 路径健壮性（D-27）
...
## Phase VI — 编辑器页面去重（有选择性）

依赖：V.6（IEditorDataSource 新接口）完成后执行。
...
### VI.1 激活 EditorPageBase，合并确定性重复代码（D-29）
...
### VI.2 删除废弃的 EditorPageBase 旧实现（D-33）
...
| 原提案 | 排除理由 |
|---|---|
| ~~VI.2 ensureEngine\<T\>() 模板~~ | MinLabel 的 ASR provider 不走通用模板，强行统一需额外 traits 层，投入产出比低 |
| ~~VI.3 InferenceRunner~~ | 三种推理的输入准备和结果处理差异太大，统一接口会过度抽象。V.6 的 `validatedAudioPath()` 已统一解决音频检查问题，各页面只需改一行 |
...
3. **V.6 音频有效性检查自动传播**：基类的 `onSliceSelected` 统一调用 `validatedAudioPath()`

***

## Phase VII — 路径管理统一与推理 UX 改进
...
### VII.5 AudioVisualizerContainer 文档/模型刷新框架（D-34）— 紧急
